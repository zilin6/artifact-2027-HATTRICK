...
/* Keep the benchmark runtime bookkeeping out of the main benchmark .bss so it
   does not share cache lines with crypto-managed working sets. */
#define RUNTIME_BSS_ATTR __attribute__ ((section (".crypto_runtime_bss")))

/* Seed for the random number generator */

static long int seed RUNTIME_BSS_ATTR __attribute__ ((aligned (64))) = 0;

/* Heap records and sane initial values */

static void *heap_ptr RUNTIME_BSS_ATTR = NULL;
static void *heap_end RUNTIME_BSS_ATTR = NULL;
static size_t heap_requested RUNTIME_BSS_ATTR = 0;
