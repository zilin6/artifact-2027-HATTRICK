...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one or more plain-VM Embench benchmarks on SmallBoomV3Config and print measured benchmark cycles from verbose marker logs."
    )
    parser.add_argument(
        "benchmarks",
        nargs="*",
    ...
    parser.add_argument(
        "--skip-build",
        action="store_true",
        help="Reuse the existing plain-VM build directory instead of running ./build.sh first.",
    )
    parser.add_argument(
        "--log-path",
