...
verify_benchmark (int res __attribute ((unused)))
{
  ...
  for (i = 0; i < 64; ++i)
    if (pInfo.m_pMCUBufR[i] != r_ref[i])
      {
        embench_debug_mismatch ('R', (unsigned) i,
                                (unsigned) pInfo.m_pMCUBufR[i],
                                (unsigned) r_ref[i]);
        return 0;
      }

  for (i = 0; i < 64; ++i)
    if (pInfo.m_pMCUBufG[i] != g_ref[i])
      {
        embench_debug_mismatch ('G', (unsigned) i,
                                (unsigned) pInfo.m_pMCUBufG[i],
                                (unsigned) g_ref[i]);
        return 0;
      }

  for (i = 0; i < 64; ++i)
    if (pInfo.m_pMCUBufB[i] != b_ref[i])
      {
        embench_debug_mismatch ('B', (unsigned) i,
                                (unsigned) pInfo.m_pMCUBufB[i],
                                (unsigned) b_ref[i]);
        return 0;
      }
