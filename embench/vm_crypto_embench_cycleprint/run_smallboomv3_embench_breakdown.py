...
class BreakdownResult:
    ...
    added_crypto_copy: Optional[int]
    warm: Optional[int]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run SmallBoomV3 embench and keep a unified cycle breakdown report."
    )
...
def parse_breakdown(bench: str, log_path: Path) -> BreakdownResult:
    ...
    added_crypto_copy = startup_end - orig_startup_end
    warm = measure_start - startup_end
    measured = verify_start - measure_start
    verify_tail = total - verify_start

    if min(startup_orig, added_crypto_copy, warm, measured, verify_tail) < 0:
        return BreakdownResult(
            bench,
            "failure",
            total,
        ...
        for row in reader:
            results.append(
                BreakdownResult(
                    ...
                    added_crypto_copy=int(row["added_crypto_copy"]) if row["added_crypto_copy"] else None,
...
def render_report(
    ...
    for bench in BENCHMARK_ORDER:
        ...
        if not breakdown or breakdown.status != "success":
            continue
        added_ratio = ""
        if breakdown.total and breakdown.added_crypto_copy is not None:
            added_ratio = f"{(breakdown.added_crypto_copy / breakdown.total) * 100:.3f}%"
        passed_rows.append([
            f"`{bench}`",
            format_cycles(breakdown.total),
            format_cycles(breakdown.startup_orig),
            format_cycles(breakdown.added_crypto_copy),
    ...
    sections = [
        ...
        "- `added crypto-copy`: extra startup cycles inserted ahead of that warm-up entry. Repos without the extra path report `0`.",
        ...
        write_tsv(
            ...
            [
                [
                    ...
                    "" if item.added_crypto_copy is None else str(item.added_crypto_copy),
