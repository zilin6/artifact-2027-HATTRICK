...
static PJPG_INLINE int16
arithmeticRightShiftN16 (int16 x, int8 n)
{
...
static PJPG_INLINE long
arithmeticRightShift8L (long x)
{
...
fillInBuf (void)
{
  ...
  status =
    (*g_pNeedBytesCallback) (gInBuf + gInBufOfs,
			     PJPG_MAX_IN_BUF_SIZE - gInBufOfs, &gInBufLeft,
			     g_pCallback_data);
  if (status)
    {
...
//------------------------------------------------------------------------------
static uint16
getBits (uint8 numBits, uint8 FFCheck)
{
...
//------------------------------------------------------------------------------
static PJPG_INLINE int16
huffExtend (uint16 x, uint8 s)
{
  return ((x <
	   getExtendTest (s)) ? ((int16) x +
				 getExtendOffset (s)) : (int16) x);
}

//------------------------------------------------------------------------------
static PJPG_INLINE uint8
huffDecode (const HuffTable * pHuffTable, const uint8 * pHuffVal)
{
  uint8 i = 0;
  uint8 j;
  uint16 code = getBit ();
...
//------------------------------------------------------------------------------
static void
huffCreate (const uint8 * pBits, HuffTable * pHuffTable)
{
  uint8 i = 0;
  uint8 j = 0;
...
//------------------------------------------------------------------------------
static HuffTable * __attribute__ ((noinline, optimize ("no-tree-switch-conversion")))
getHuffTable (uint8 index)
{
  if (index == 0)
    return &gHuffTab0;
  if (index == 1)
...
//------------------------------------------------------------------------------
static uint8 * __attribute__ ((noinline, optimize ("no-tree-switch-conversion")))
getHuffVal (uint8 index)
{
  if (index == 0)
    return gHuffVal0;
  if (index == 1)
...
static uint8
locateSOSMarker (uint8 * pFoundEOI)
{
  uint8 c;
  uint8 status;
...
static void
createWinogradQuant (int16 * pQuant)
{
  uint8 i;
	...
	(int16) ((x +
		  (1 <<
		   (PJPG_WINOGRAD_QUANT_SCALE_BITS - PJPG_DCT_SCALE_BITS -
		    1))) >> (PJPG_WINOGRAD_QUANT_SCALE_BITS -
			     PJPG_DCT_SCALE_BITS));
    }
}
...
static void
upsampleCbH (uint8 srcOfs, uint8 dstOfs)
{
  // Cb - affects G and B
  uint8 x, y;
  int16 *pSrc = gCoeffBuf + srcOfs;
...
static void
upsampleCr (uint8 srcOfs, uint8 dstOfs)
{
  // Cr - affects R and G
  uint8 x, y;
  int16 *pSrc = gCoeffBuf + srcOfs;
...
static void
upsampleCrH (uint8 srcOfs, uint8 dstOfs)
{
  // Cr - affects R and G
  uint8 x, y;
  int16 *pSrc = gCoeffBuf + srcOfs;
...
static void
upsampleCrV (uint8 srcOfs, uint8 dstOfs)
{
  // Cr - affects R and G
  uint8 x, y;
  int16 *pSrc = gCoeffBuf + srcOfs;
...
//------------------------------------------------------------------------------
static void
transformBlockReduce (uint8 mcuBlock)
{
  ...
  for (mcuBlock = 0; mcuBlock < gMaxBlocksPerMCU; mcuBlock++)
    {
      ...
      uint8 s =
	huffDecode (compDCTab ? &gHuffTab1 : &gHuffTab0,
      ...
      if (gReduce)
	{
	  ...
	  for (k = 1; k < 64; k++)
	    {
	      s =
		huffDecode (compACTab ? &gHuffTab3 : &gHuffTab2,
