...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one or more vm_crypto_embench benchmarks on SmallBoomV3Config and print measured benchmark cycles from verbose marker logs when available."
    )
    parser.add_argument(
        "benchmarks",
        nargs="*",
    ...
    parser.add_argument(
        "--log-path",
        help="Optional explicit path for the simulator log. Only valid when running exactly one benchmark.",
    )
    parser.add_argument(
        "--no-verbose",
        action="store_true",
        help="Do not pass +verbose to the simulator. Marker-derived benchmark cycles will be unavailable.",
    )
    parser.add_argument(
        "--summary-tsv",
...
def run_one_benchmark(bench: str, timeout_cycles: int, log_path: Path, verbose: bool) -> RunResult:
    binary = embench_root() / "build" / bench
    if not binary.is_file():
        return RunResult(bench, "failure", None, None, None, None, log_path, f"missing binary: {binary}")

    extra_sim_flags = "+verbose" if verbose else ""
    cmd = (
        f"{shell_prefix()}"
        ...
        f"EXTRA_SIM_FLAGS={extra_sim_flags} "
        f"run-binary-fast"
    )
    ...
    if not verbose:
        return RunResult(
            ...
            log_path,
            "non-verbose run; marker cycles unavailable",
        )
...
def main() -> int:
    ...
    for bench in benchmarks:
        ...
        result = run_one_benchmark(bench, timeout_cycles, log_path, not args.no_verbose)
