...
OUTDIR="$ROOT/software/embench/run-logs-${RUN_TS_FILE}-smallboomv3-verbose-bg"
RUNNER_LOG="$ROOT/embench-verbose-bg-${RUN_TS_FILE}.log"
PID_FILE="$ROOT/.latest_embench_verbose_bg_pid"
OUTDIR_FILE="$ROOT/.latest_embench_verbose_bg_dir"
...
results_to_markdown() {
  ...
  {
    echo
    echo "## Background Verbose Embench Rerun ($RUN_TS_HUMAN)"
    echo
    echo "- Config: \`$CONFIG\`"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S %z')] starting verbose Embench rerun" >> "$RUNNER_LOG"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S %z')] completed verbose Embench rerun" >> "$RUNNER_LOG"
