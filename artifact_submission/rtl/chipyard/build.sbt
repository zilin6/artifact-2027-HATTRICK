...
// Artifact-only build mode used for SmallBoomV3Config reproduction.
// It avoids compiling unrelated optional generators whose upstream
// Chipyard versions are not part of the submitted SmallBoomV3 design.
lazy val artifactSmallBoomOnly = sys.env.get("CHIPYARD_ARTIFACT_SMALLBOOM_ONLY").exists { value =>
...
lazy val chipyard = {
  ...
  val baseProjects: Seq[ProjectReference] = alwaysOnBaseProjects

  val baseDeps: Seq[sbt.ClasspathDep[sbt.ProjectReference]] =
    baseProjects.map(pr => sbt.ClasspathDependency(pr, None))

  // Optional settings to exclude specific sources under Chisel 7 or artifact SmallBoom-only builds.
  val chipyardSourceExcludeSettings: Seq[Def.Setting[_]] = if (useChisel7 || smallBoomOnly) Seq(
    Compile / unmanagedSources := {
      val files = (Compile / unmanagedSources).value
      val root = (ThisBuild / baseDirectory).value
      val chisel7ExcludeList = if (useChisel7) Seq(
      ...
      val artifactSmallBoomExcludeList = if (smallBoomOnly) Seq(
      ...
      val excludeList = (chisel7ExcludeList ++ artifactSmallBoomExcludeList).distinct.map(p => (root / p).getCanonicalFile)
  ...
  var cy = Project(id = "chipyard", base = file("generators/chipyard"))
    ...
    .settings(Compile / unmanagedSourceDirectories ++= {
      if (smallBoomOnly) Seq(file("generators/chipyard/src/main/scala_artifact_smallboom"))
      else Seq.empty
    })
    .settings(chipyardSourceExcludeSettings: _*)
  ...
  val discovered = if (smallBoomOnly) Seq.empty else optionalModules.filter { case (dir, _) =>
...
lazy val rocketchip_inclusive_cache_toptests = (project in file("generators/rocket-chip-inclusive-cache-toptests"))
  .dependsOn(rocketchip_inclusive_cache, boom, chipyard)
  .settings(commonSettings)
  .settings(chiselSettings)
  .settings(
    Compile / unmanagedSourceDirectories := Seq.empty,
    Test / scalaSource := (ThisBuild / baseDirectory).value / "generators" / "rocket-chip-inclusive-cache" / "src" / "test" / "scala" / "sifive" / "blocks" / "inclusivecache" / "top",
