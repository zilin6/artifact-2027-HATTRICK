...
static inline uint64_t align_to_line(uint64_t addr, uint64_t line_size)
{
  ...
  bool b_ready)
{
  ...
  if (mem->willAcceptTransaction()) {
    for (auto it = rreq_queue.begin(); it != rreq_queue.end(); it++) {
      if (!read_id_busy[it->id]) {
        ...
        auto transaction = *it;
        auto line_addr = align_to_line(transaction.addr, line_size);
        rreq[line_addr].push(transaction);
  ...
  if (aw_fire) {
    store_addr = aw_addr;
    store_line_addr = align_to_line(aw_addr, line_size);
    store_id = aw_id;
  ...
  if (w_fire) {
    ...
    if (store_count == 0) {
      ...
      mem->addTransaction(true, store_line_addr);
      wreq[store_line_addr].push(store_id);
      assert(w_last);
    }
  }
