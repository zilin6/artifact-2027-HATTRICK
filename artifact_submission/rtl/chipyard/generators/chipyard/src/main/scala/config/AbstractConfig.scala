...
import freechips.rocketchip.rocket.{CacheCryptoDebugLogKey, CacheCryptoDebugLogParams, CacheCryptoEnableKey, CacheCryptoNonceBitsKey, CacheCryptoCounterBitsKey}

class WithCacheCrypto(
  enable: Boolean = false,
  nonceBits: Int = 64,
  counterBits: Int = 64) extends Config((site, here, up) => {
  case CacheCryptoEnableKey => enable
  case CacheCryptoNonceBitsKey => nonceBits
  case CacheCryptoCounterBitsKey => counterBits
})

class WithCacheCryptoDebugLog(
  plusArgName: String = "cache_crypto_debug_log",
  defaultEnable: Int = 0) extends Config((site, here, up) => {
  case CacheCryptoDebugLogKey => CacheCryptoDebugLogParams(
    plusArgName = plusArgName,
...
class AbstractConfig extends Config(
  ...
  // new chipyard.config.WithRom_4_os_image++                               /*os*/
