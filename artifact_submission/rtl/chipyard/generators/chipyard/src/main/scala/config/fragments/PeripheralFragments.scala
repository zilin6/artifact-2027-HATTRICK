...
import freechips.rocketchip.devices.tilelink.{BootROMLocated, PLICKey, CLINTKey,MaskROMLocated,MaskROMParams}
...
class WithRom_4_os_image(address : BigInt = 0x20000,sizeBytes: Int = 0x80,width: Int = 32) extends Config((site, here, up) => {
  case MaskROMLocated(loc) => 
      // val bytesPerWord = width / 8
      // val depth = (sizeBytes + bytesPerWord - 1) / bytesPerWord
      up(MaskROMLocated(loc), site) :+ MaskROMParams(address = address, name = "os_image_rom", depth = 32,width = width)
})
