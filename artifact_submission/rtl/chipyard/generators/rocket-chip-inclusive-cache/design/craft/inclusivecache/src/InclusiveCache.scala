...
import freechips.rocketchip.rocket.{CacheCryptoConfigBundle, CacheCryptoRefillMeta, CacheCryptoRefillMetaField, CacheCryptoWritebackMeta}
import freechips.rocketchip.subsystem.{SubsystemBankedCoherenceKey}
import freechips.rocketchip.regmapper._
import freechips.rocketchip.tilelink._
import freechips.rocketchip.util.{PlusArg, plusarg_reader}
    ...
    extends LazyModule
{
  ...
  val outerGetPutAccess = TransferSizes(8, cache.blockBytes)
  ...
  val node: TLAdapterNode = TLAdapterNode(
    clientFn  = { _ =>
      TLMasterPortParameters.v2(masters = Seq(
        TLMasterParameters.v2(
          name = s"L${cache.level} InclusiveCache",
          sourceId = IdRange(0, 4 * (1 << log2Ceil(InclusiveCacheParameters.all_mshrs(cache, micro)))),
          ...
          emits = TLMasterToSlaveTransferSizes(
            ...
            arithmetic = TransferSizes.none,
            logical = TransferSizes.none,
            get = outerGetPutAccess,
            putFull = outerGetPutAccess,
            putPartial = TransferSizes.none,
            hint = TransferSizes.none))))
    },
    managerFn = { m => TLManagerPortParameters(
      ...
      responseFields = Seq(CacheCryptoRefillMetaField(
        counterBits = p(freechips.rocketchip.rocket.CacheCryptoCounterBitsKey))),
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val cusBaseAddressReg = RegInit(0.U(64.W))
    val preloadCounterBaseEnable = PlusArg("cache_crypto_base_preload_enable", default = 0, width = 1)
    val preloadCounterBaseValue = PlusArg("cache_crypto_base_preload_value", default = 0, width = 64)
    val preloadCounterBaseLoaded = RegInit(false.B)
    when (io.cus_base_wen) {
      cusBaseAddressReg := io.cus_base_address
      preloadCounterBaseLoaded := true.B
    } .elsewhen (!preloadCounterBaseLoaded && preloadCounterBaseEnable.orR) {
      cusBaseAddressReg := preloadCounterBaseValue
      preloadCounterBaseLoaded := true.B
    }

    val l2CryptoAssertEnableReader = Module(new plusarg_reader(
      "l2_crypto_assert_enable=%d",
      0,
      "Enable synthesized L2 crypto assert/watchdog checks",
      1))
    SynthesizedPlusArg(l2CryptoAssertEnableReader)
    val l2CryptoAssertEnable = l2CryptoAssertEnableReader.io.out =/= 0.U
    ...
    val mods = (node.in zip node.out) map { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      scheduler.io.cus_base_address := cusBaseAddressReg
      scheduler.io.l2_crypto_assert_enable := l2CryptoAssertEnable
      scheduler.io.in <> in
