...
class WithInclusiveCache(
  ...
  case SubsystemBankedCoherenceKey => up(SubsystemBankedCoherenceKey, site).copy(coherenceManager = { context =>
    ...
    val l2 = LazyModule(new InclusiveCache(
      ...
      l2Ctrl))
    val cacheCryptoConfigSinkNode = context match {
      case c: DefaultHierarchicalElementContextType =>
        val sink = org.chipsalliance.diplomacy.bundlebridge.BundleBridgeSink[CacheCryptoConfigBundle]()
        sink := c.tileCacheCryptoConfigNexusNode
        Some(sink)
      case _ => None
    }

    InModuleBody {
      l2.module.io.cus_base_address := cacheCryptoConfigSinkNode.map(_.bundle.base).getOrElse(0.U(64.W))
      l2.module.io.cus_base_wen := cacheCryptoConfigSinkNode.map(_.bundle.base_wen).getOrElse(false.B)
    }
