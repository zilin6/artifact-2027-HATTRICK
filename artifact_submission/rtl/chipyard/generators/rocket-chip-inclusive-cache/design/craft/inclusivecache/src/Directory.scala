...
class CounterSidecarAddress(params: InclusiveCacheParameters) extends InclusiveCacheBundle(params)
{
  val set = UInt(params.setBits.W)
  val way = UInt(params.wayBits.W)
}

class CounterSidecarWrite(params: InclusiveCacheParameters) extends CounterSidecarAddress(params)
{
...
class Directory(params: InclusiveCacheParameters) extends Module
{
  ...
  val reqCryptoLine = params.dirReg(RegEnable(io.read.bits.cryptoLine, ren), ren1)
  ...
  val invalidWays = Cat(ways.map(_.state === INVALID).reverse)
  val hasInvalidWay = invalidWays.orR

  val tagOnlyHits = Cat(ways.zipWithIndex.map { case (w, i) =>
    w.tag === tag &&
    w.state =/= INVALID &&
    (!setQuash || i.U =/= bypass.way)
  }.reverse)
  val tagOnlyHit = tagOnlyHits.orR

  val hits = Cat(ways.zipWithIndex.map { case (w, i) =>
    w.tag === tag &&
    w.state =/= INVALID &&
    w.cryptoLine === reqCryptoLine &&
    reqCryptoLine === w.counterValid &&
    (!setQuash || i.U =/= bypass.way)
  }.reverse)
  val hit = hits.orR

  val bypassTagOnlyHit = setQuash && tagMatch && bypass.data.state =/= INVALID
  val bypassFullHit =
    bypassTagOnlyHit &&
    bypass.data.cryptoLine === reqCryptoLine &&
    reqCryptoLine === bypass.data.counterValid

  val forceReplaceTagHit = tagOnlyHit && !hit
  val forceReplaceBypassTagHit = bypassTagOnlyHit && !bypassFullHit

  val baseResultMeta =
    Mux(hit, Mux1H(hits, ways),
      Mux(setQuash && (tagMatch || wayMatch), bypass.data, Mux1H(victimWayOH, ways)))
  val baseResultWay =
    Mux(hit, OHToUInt(hits), Mux(setQuash && tagMatch, bypass.way, victimWay))

  val resultMeta =
    Mux(forceReplaceTagHit, Mux1H(tagOnlyHits, ways),
      Mux(forceReplaceBypassTagHit, bypass.data, baseResultMeta))
  val resultWay =
    Mux(forceReplaceTagHit, OHToUInt(tagOnlyHits),
      Mux(forceReplaceBypassTagHit, bypass.way, baseResultWay))

  io.result.valid := ren2
  io.result.bits.viewAsSupertype(chiselTypeOf(bypass.data)) := resultMeta
  io.result.bits.hit := hit || (setQuash && tagMatch && bypass.data.state =/= INVALID)
  io.result.bits.way := resultWay
