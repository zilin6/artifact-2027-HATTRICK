...
class PutBufferAEntry(params: InclusiveCacheParameters) extends InclusiveCacheBundle(params)
{
  ...
  val corrupt = Bool()
  // A 路 payload 同时携带 line-level counter；同一 transaction 的各个 beat 共享同一份值。
...
class SinkA(params: InclusiveCacheParameters) extends Module
{
  ...
  val aAddr64 =
    if (params.inner.bundle.addressBits < 64) {
      Cat(0.U((64 - params.inner.bundle.addressBits).W), a.bits.address)
    } else {
      a.bits.address
    }
  val put = Mux(first, freeIdx, RegEnable(freeIdx, first))
  val aWritebackMeta = a.bits.user.lift(CacheCryptoWritebackMeta)
  val aCryptoLine = WireDefault(false.B)
  val aCounter = WireDefault(0.U(params.outer.bundle.dataBits.W))
  aWritebackMeta.foreach { m =>
    aCryptoLine := m.cryptoLine
    aCounter := m.counter
  }
  val debugDataRangeReq =
    aAddr64 >= debugDataWatchStart &&
    aAddr64 < debugDataWatchEnd
  ...
  io.req.bits.tag    := tag
  io.req.bits.put    := put
  io.req.bits.cryptoLine := aCryptoLine
  ...
  putbuffer.io.push.bits.data.counter := aCounter
  putbuffer.io.push.bits.data.counterValid := aCryptoLine

  when (a.fire && first && hasData && aCryptoLine) {
    chisel3.printf("[SINKA-CTR-SET] source=%d set=0x%x put=0x%x first=%d opcode=0x%x size=%d counter=0x%x\n",
      a.bits.source,
      set,
      put,
      first,
  ...
  when (io.pb_pop.fire && io.pb_pop.bits.last) {
    chisel3.printf("[SINKA-CTR-CLR] pop_idx=0x%x last=%d ctr_valid_before=%d counter=0x%x\n",
      io.pb_pop.bits.index,
      io.pb_pop.bits.last,
