...
class BoomDCacheSameSetMissSpacingSpec extends AnyFlatSpec with ChiselScalatestTester with Matchers {
  ...
  private def issueLoadMiss(dut: BoomDCacheRealDriverHarnessWrapper, address: BigInt): Unit = {
  ...
  private def waitOuterAcquire(dut: BoomDCacheRealDriverHarnessWrapper, maxCycles: Int = 200): (BigInt, BigInt) = {
    val outer = dut.io.outer
    var cycles = 0
  ...
  behavior of "Boom V3 DCache realistic miss spacing"

  it should "nack and delay a second same-set different-tag miss until the first miss completes" in {
    test(new BoomDCacheRealDriverHarnessWrapper)
      ...
      val observedSecondSource = retryLoadUntilOuterAcquire(dut, secondAddr, maxAttempts = 24, observeCyclesPerAttempt = 64)

      respondGrantData(dut, observedSecondSource, BigInt("5000000000000000", 16))
      waitResp(dut, maxCycles = 400)
    }
  }
}
