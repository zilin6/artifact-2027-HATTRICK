  ...
  with boom.v3.common.HasBoomCoreParameters
{
  ...
  // 被 branch mispredict 或 flush 杀掉的 entry
...
/// key和 value 分别是多少位呢？  首先 低位 12bit不变 高位 24bit 进行 tlb 翻译 
// shadow tlb 
class C4_TLB(implicit p : Parameters) extends BoomModule {
  val io = IO(new Bundle {
    val write = Flipped(Valid(new Bundle {
      val key = UInt(26.W)
      val value = UInt(26.W)
    }))
    val lookup = Vec(2, Flipped(Valid(UInt(vaddrBitsExtended.W))))
    val result = Vec(2, Valid(UInt(vaddrBitsExtended.W)))
    val flush = Input(Bool())
  })
  private def printf(args: Any*): Unit = {}
  // 用于存储 key-value 对的简单数组
  // val entries = Reg(Vec(2, new{
  // val key = Vec(2, Reg(UInt(24.W)))
  val key = Reg(Vec(2,UInt(26.W)))
  val value = Reg(Vec(2,UInt(26.W)))
  val entry_valid = RegInit(VecInit(Seq.fill(2){false.B}))
  // }))
  ...
  when(io.write.valid) {
    // 查找空闲槽位或者替换第一个条目
    // val freeSlot = entries.indexWhere(!_.valid)
    val free_index = entry_valid.indexWhere(!_) 
    when(free_index >= 0.U) {
      key(free_index) := io.write.bits.key
      value(free_index) := io.write.bits.value
      entry_valid(free_index) := true.B
      printf("insert key : 0x%x value : 0x%x\n",io.write.bits.key,io.write.bits.value)
      printf("vaddrBitsExtended : %d",vaddrBitsExtended.U);
    }.otherwise{
      key(0) := io.write.bits.key
      value(0) := io.write.bits.value
      entry_valid(0) := true.B
    }
  }
  ...
  {
    printf("tlb flush\n")
    entry_valid(0) := false.B
    entry_valid(1) := false.B
  }

  // 查找操作
  for (j <- 0 until 2) 
  {
    when(io.lookup(j).valid) 
    {
      // printf("lookup addr : 0x%x\n",io.lookup(j).bits)
      for (i <- 0 until 2) 
      {
        // when (entry_valid(i))
        // {
        //   printf("key [%d] : 0x%x\n", i.U,key(i))
        // }
        when(entry_valid(i) && (key(i) === io.lookup(j).bits(37,12))) 
        {
          io.result(j).bits := Cat( io.lookup(j).bits(vaddrBitsExtended-1,38),value(i), io.lookup(j).bits(11,0))
          // printf("tlb result [%d]: 0x%x\n",j.U,Cat( io.lookup(j).bits(vaddrBitsExtended-1,38),value(i), io.lookup(j).bits(11,0)))
          io.result(j).valid := true.B
        }
      }
    }
  }
...
class C4_frontend_TLB(implicit p : Parameters) extends BoomModule {
  val io = IO(new Bundle {
    val write = Flipped(Valid(new Bundle {
      val key = UInt(26.W)
      val value = UInt(26.W)
    ...
    val result = Vec(2, Valid(UInt(vaddrBitsExtended.W)))
    val flush = Input(Bool())
    val fetch_addr_req = DecoupledIO(new addr_req)
  })
  private def printf(args: Any*): Unit = {}
  private def feTlbPrintf(msg: Printable): Unit = { _root_.chisel3.printf(msg) }
  private val runtimeVirtLo = "h40001000".U(vaddrBitsExtended.W)
  private val runtimeVirtHi = "h40002000".U(vaddrBitsExtended.W)
  private val probeVirtLo   = "h40006000".U(vaddrBitsExtended.W)
  private val probeVirtHi   = "h4000a000".U(vaddrBitsExtended.W)

  private def watchLookup(addr: UInt): Bool =
    (addr >= runtimeVirtLo && addr < runtimeVirtHi) ||
    (addr >= probeVirtLo && addr < probeVirtHi)

  // 用于存储 key-value 对的简单数组
  // val entries = Reg(Vec(2, new{
  // val key = Vec(2, Reg(UInt(24.W)))
  val repl_ptr = RegInit(0.U(1.W))
  val key = Reg(Vec(2,UInt(26.W)))
  val value = Reg(Vec(2,UInt(26.W)))
  val entry_valid = RegInit(VecInit(Seq.fill(2){false.B}))
  ...
  when(io.write.valid) {
    ...
    // val freeSlot = entries.indexWhere(!_.valid)
    // val free_index = entry_valid.indexWhere(!_) 
    when(entry_valid(0) === false.B) {
      key(0) := io.write.bits.key
      value(0) := io.write.bits.value
      entry_valid(0) := true.B
      // printf("fetch tlb index 0 insert key : 0x%x value : 0x%x\n",io.write.bits.key,io.write.bits.value)
    }.elsewhen(entry_valid(1) === false.B){
      key(1) := io.write.bits.key
      value(1) := io.write.bits.value
      entry_valid(1) := true.B
      // printf("fetch tlb index 1 insert key : 0x%x value : 0x%x\n",io.write.bits.key,io.write.bits.value)
    }.otherwise{
      key(repl_ptr) := io.write.bits.key
      value(repl_ptr) := io.write.bits.value
      entry_valid(repl_ptr) := true.B
      repl_ptr := ~repl_ptr
      // printf("fetch tlb index %d insert key : 0x%x value : 0x%x\n",repl_ptr,io.write.bits.key,io.write.bits.value)
    }
    val origPage = Cat(io.write.bits.key, 0.U(12.W))
    val encPage = Cat(io.write.bits.value, 0.U(12.W))
    when (watchLookup(origPage)) {
      feTlbPrintf(p"[C4-FE-WRITE] key=0x${Hexadecimal(io.write.bits.key)} orig_page=0x${Hexadecimal(origPage)} " +
        p"enc_page=0x${Hexadecimal(encPage)} value=0x${Hexadecimal(io.write.bits.value)} " +
        p"v0=${entry_valid(0).asUInt} v1=${entry_valid(1).asUInt} repl=${repl_ptr}\n")
    }
  }
  ...
  val s0Miss = io.lookup(0).valid && !io.result(0).valid
  val s1Miss = io.lookup(1).valid && !io.result(1).valid
  val hasSel = s0Miss || s1Miss

  io.fetch_addr_req.valid := false.B
  io.fetch_addr_req.bits  := DontCare

  when (hasSel) {
    // Frontend may miss on both s0 and the older s1 in the same cycle.
    // Prioritize s1 so we never merge two different virtual addresses.
    io.fetch_addr_req.valid     := true.B
    io.fetch_addr_req.bits.addr := Mux(s1Miss, io.lookup(1).bits, io.lookup(0).bits)
    io.fetch_addr_req.bits.data := 0.U
    io.fetch_addr_req.bits.data_valid := false.B
    when (watchLookup(io.fetch_addr_req.bits.addr)) {
      feTlbPrintf(p"[C4-FE-MISS] sel=${Mux(s1Miss, 1.U, 0.U)} s0_lookup_v=${io.lookup(0).valid.asUInt} " +
        p"s0_addr=0x${Hexadecimal(io.lookup(0).bits)} s0_hit=${io.result(0).valid.asUInt} " +
        p"s1_lookup_v=${io.lookup(1).valid.asUInt} s1_addr=0x${Hexadecimal(io.lookup(1).bits)} " +
        p"s1_hit=${io.result(1).valid.asUInt} req_addr=0x${Hexadecimal(io.fetch_addr_req.bits.addr)}\n")
    }
  }

  for (j <- 0 until 2) {
    when (io.lookup(j).valid && io.result(j).valid && watchLookup(io.lookup(j).bits)) {
      feTlbPrintf(p"[C4-FE-HIT] lane=${j.U} orig_addr=0x${Hexadecimal(io.lookup(j).bits)} " +
        p"enc_addr=0x${Hexadecimal(io.result(j).bits)}\n")
    }
  }
