  ...
  with IssueUnitConstants
{
  ...
  private def dbgHangStorePc(pc: UInt): Bool = {
    pc === "h40000dda".U(coreMaxAddrBits.W) ||
    pc === "h80000dda".U(coreMaxAddrBits.W)
  }
  ...
  } .elsewhen (io.grant && (state === s_valid_2)) 
  {
    when (!(io.ldspec_miss && (p1_poisoned || p2_poisoned))) 
    {
      // 标记为s_valid_1（表示还有一个STD微操作需要发射）。
      next_state := s_valid_1
      when (p1) {
        // 如果第一个操作数就绪（p1为真），则将该微操作转换为uopSTD（Store Data）微操作
  ...
  when (io.in_uop.valid && dbgHangStorePc(io.in_uop.bits.debug_pc)) {
    printf("[HANG-IQ-SLOT-IN] pc=0x%x inst=0x%x rob=%d uopc=%d iw_state=%d lrs1_t=%d lrs2_t=%d prs1=%d prs2=%d prs1_busy=%d prs2_busy=%d clear=%d kill=%d\n",
      io.in_uop.bits.debug_pc,
      io.in_uop.bits.debug_inst,
      io.in_uop.bits.rob_idx,
      io.in_uop.bits.uopc,
      io.in_uop.bits.iw_state,
      io.in_uop.bits.lrs1_rtype,
      io.in_uop.bits.lrs2_rtype,
      io.in_uop.bits.prs1,
      io.in_uop.bits.prs2,
  ...
  when (state === s_valid_2) 
  {
    when (p1 && p2 && ppred) {
      ...
      // 这里会将 lrs2_rtype设置为RT_X，这样func decode 在处理的时候就会将这个microOp值当作 sta,不会变成stad
  ...
  when ((io.grant || io.clear || io.kill) && dbgHangStorePc(slot_uop.debug_pc)) {
    printf("[HANG-IQ-SLOT-XFER] pc=0x%x inst=0x%x rob=%d state=%d grant=%d clear=%d kill=%d req=%d p1=%d p2=%d p3=%d ppred=%d uopc_cur=%d uopc_issue=%d next_state=%d next_uopc=%d next_lrs1_t=%d next_lrs2_t=%d will_valid=%d in_valid=%d in_pc=0x%x in_uopc=%d in_state=%d\n",
      slot_uop.debug_pc,
      slot_uop.debug_inst,
      slot_uop.rob_idx,
      state,
