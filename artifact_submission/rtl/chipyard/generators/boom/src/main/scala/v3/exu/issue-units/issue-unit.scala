  ...
  with IssueUnitConstants
{
  ...
  for (w <- 0 until dispatchWidth) {
    ...
    when ((iqType == IQT_MEM.litValue).B && io.dis_uops(w).valid && dbgHangStorePc(io.dis_uops(w).bits.debug_pc)) {
      printf("[HANG-IQ-IN] lane=%d pc=0x%x inst=0x%x rob=%d uopc_in=%d uopc_eff=%d is_sta=%d is_std=%d uses_stq=%d lrs1_t=%d lrs2_t=%d prs1=%d prs2=%d prs1_busy=%d prs2_busy=%d iw_state=%d brmask=0x%x\n",
        w.U,
        io.dis_uops(w).bits.debug_pc,
        io.dis_uops(w).bits.debug_inst,
        io.dis_uops(w).bits.rob_idx,
        io.dis_uops(w).bits.uopc,
        dis_uops(w).uopc,
        io.dis_uops(w).bits.ctrl.is_sta,
        io.dis_uops(w).bits.ctrl.is_std,
        io.dis_uops(w).bits.uses_stq,
        dis_uops(w).lrs1_rtype,
        dis_uops(w).lrs2_rtype,
        dis_uops(w).prs1,
        dis_uops(w).prs2,
        dis_uops(w).prs1_busy,
        dis_uops(w).prs2_busy,
        dis_uops(w).iw_state,
        dis_uops(w).br_mask)
    }
  }
  ...
  for (i <- 0 until numIssueSlots) 
  {
    //when (issue_slots(i).uop.debug_pc(31,16) === 0x8000.U)
    when (io.log )
    { 
      printf(this.getType + "    Slot[%d]: " +
