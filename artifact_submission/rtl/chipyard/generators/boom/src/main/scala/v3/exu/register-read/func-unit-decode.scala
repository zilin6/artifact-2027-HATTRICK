...
object AluRRdDecode extends RRdDecodeConstants
{
  val table: Array[(BitPat, List[BitPat])] =
         ...
         BitPat(uop_gen_key) -> List(BR_N , Y, N, N, FN_ADD , DW_XPR, OP1_ZERO, OP2_ZERO, IS_X, REN_0, CSR.N),
...
object MemRRdDecode extends RRdDecodeConstants
{
  val table: Array[(BitPat, List[BitPat])] =
         ...
         BitPat(uop_load_key) -> List(BR_N , N, N, Y, FN_ADD , DW_XPR, OP1_RS1 , OP2_IMM , IS_I, REN_0, CSR.N),
         BitPat(uopSTA)   -> List(BR_N , N, N, Y, FN_ADD , DW_XPR, OP1_RS1 , OP2_IMM , IS_S, REN_0, CSR.N),
         BitPat(uop_store_key) -> List(BR_N , N, N, Y, FN_ADD , DW_XPR, OP1_RS1 , OP2_IMM , IS_S, REN_0, CSR.N),
  ...
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  ...
  io.rrd_uop.ctrl.is_load := io.rrd_uop.uopc === uopLD || io.rrd_uop.uopc === uop_load_key
  io.rrd_uop.ctrl.is_sta  := io.rrd_uop.uopc === uopSTA || io.rrd_uop.uopc === uopAMO_AG || io.rrd_uop.uopc === uop_store_key
