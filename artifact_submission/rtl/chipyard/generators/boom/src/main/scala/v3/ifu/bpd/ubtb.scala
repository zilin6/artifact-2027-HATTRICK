...
import boom.v3.util.{BoomCoreStringPrefix, WrapInc, BoolToChar}
...
class MicroBTBBranchPredictorBank(params: BoomMicroBTBParams)(implicit p: Parameters) extends BranchPredictorBank()(p)
{
  ...
  val s1_req_rbtb  = VecInit(btb.read(s0_lookup_idx , s0_valid).map(_.asTypeOf(new MicroBTBEntry)))
  val s1_req_rmeta = VecInit(meta.read(s0_lookup_idx, s0_valid).map(_.asTypeOf(new MicroBTBMeta)))
  ...
  // val ubtbWatchLo = "h8000230".U(vaddrBitsExtended.W)
  // val ubtbWatchHi = "h8000240".U(vaddrBitsExtended.W)
  // def inUbtbWatch(pc: UInt): Bool = pc >= ubtbWatchLo && pc < ubtbWatchHi
  // val ubtbWatch =
  //   (s1_valid && inUbtbWatch(s1_pc)) ||
  //   (s1_update.valid && (inUbtbWatch(s1_update.bits.pc) || inUbtbWatch(s1_update.bits.target) || s1_update.bits.target === 0.U))
  ...
  val s1_taken_mask    = Cat(s1_taken.reverse)
  val s1_resp_br_mask  = Cat(s1_is_br.reverse)
  val s1_resp_jal_mask = Cat(s1_is_jal.reverse)

  // when (ubtbWatch && s1_valid) {
  //   printf("[UBTB][READ] s1_pc=0x%x s1_idx=0x%x s1_tag=0x%x\n", s1_pc, s1_idx, s1_req_tag(tagSz-1,0))
  //   for (w <- 0 until bankWidth) {
  //     printf("[UBTB][READ] slot=%d tag=0x%x ctr=0x%x is_br=%c respV=%c resp=0x%x taken=%c is_jal=%c\n",
  //       w.U,
  //       s1_req_rmeta(w).tag,
  //       s1_req_rmeta(w).ctr,
  //       BoolToChar(s1_req_rmeta(w).is_br,'T'),
  //       BoolToChar(s1_resp(w).valid,'T'),
  //       s1_resp(w).bits,
  //       BoolToChar(s1_taken(w),'T'),
  //       BoolToChar(s1_is_jal(w),'T'))
  //   }
  // }
  ...
  val wrbypass_hits = VecInit((0 until nWrBypassEntries) map { i =>
    !doing_reset &&
    wrbypass_idxs(i) === s1_update_lookup_idx(log2Ceil(nSets)-1,0)
  })
  ...
  for (w <- 0 until bankWidth) {
    s1_update_wmeta_data(w).tag     := s1_update_lookup_idx >> log2Ceil(nSets)
  ...
  when (s1_update_wmeta_mask =/= 0.U) {
    when (wrbypass_hit) {
      ...
      wrbypass_idxs(wrbypass_enq_idx) := s1_update_lookup_idx
      wrbypass_enq_idx := WrapInc(wrbypass_enq_idx, nWrBypassEntries)
    }
  }

  when (io.log && s1_valid) {
    printf(p"[UBTB][LOOKUP] pc=0x${Hexadecimal(s1_pc)} cpc_v=${s1_cvalid.asUInt} cpc=0x${Hexadecimal(s1_cpc)} idx=0x${Hexadecimal(s1_lookup_idx)} tag=0x${Hexadecimal(s1_req_tag(tagSz-1,0))} resp_v=0x${Hexadecimal(s1_resp_v_mask)} taken=0x${Hexadecimal(s1_taken_mask)} br_mask=0x${Hexadecimal(s1_resp_br_mask)} jal_mask=0x${Hexadecimal(s1_resp_jal_mask)}\n")
  }

  when (io.log && s1_update.valid) {
    printf(p"[UBTB][UPDATE] pc=0x${Hexadecimal(s1_update.bits.pc)} cpc_v=${s1_update.bits.cpc_valid.asUInt} cpc=0x${Hexadecimal(s1_update.bits.cpc)} idx=0x${Hexadecimal(s1_update_lookup_idx)} target=0x${Hexadecimal(s1_update.bits.target)} cfi_v=${s1_update.bits.cfi_idx.valid.asUInt} cfi_idx=${s1_update.bits.cfi_idx.bits} taken=${s1_update.bits.cfi_taken.asUInt} br_mask=0x${Hexadecimal(s1_update.bits.br_mask)} wbtb_mask=0x${Hexadecimal(s1_update_wbtb_mask)} wmeta_mask=0x${Hexadecimal(s1_update_wmeta_mask)} offset=0x${Hexadecimal(new_offset_value.asUInt)} wrbypass_hit=${wrbypass_hit.asUInt} wrbypass_hit_idx=${wrbypass_hit_idx} wrbypass_enq_idx=${wrbypass_enq_idx}\n")
  }

  // when (ubtbWatch && s1_update.valid) {
  //   printf("[UBTB][UPD] pc=0x%x target=0x%x idx=0x%x cfiV=%c cfi_idx=%d taken=%c br_mask=0x%x off=0x%x wbtb_mask=0x%x wmeta_mask=0x%x commit=%c\n",
  //     s1_update.bits.pc,
