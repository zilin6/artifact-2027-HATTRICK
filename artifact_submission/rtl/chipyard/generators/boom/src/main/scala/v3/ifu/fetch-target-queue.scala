  ...
  with HasBoomFrontendParameters
{
  ...
  val io = IO(new BoomBundle {
    ...
    val ras_update_cpc = Output(UInt(vaddrBitsExtended.W))
    val ras_update_cpc_valid = Output(Bool())
  ...
  val cpcs     = Reg(Vec(num_entries, UInt(vaddrBitsExtended.W)))
  val cpc_valids = RegInit(VecInit(Seq.fill(num_entries)(false.B)))
  ...
  when (do_enq) {
    ...
    cpcs(enq_ptr)          := io.enq.bits.cpc
    cpc_valids(enq_ptr)    := io.enq.bits.cpc_valid
    ...
    new_entry.addr_crypto_mode := io.enq.bits.addr_crypto_mode
    new_entry.ras_top       := io.enq.bits.ras_top
    new_entry.ras_top_cpc   := io.enq.bits.ras_top_cpc
    new_entry.ras_top_cpc_valid := io.enq.bits.ras_top_cpc_valid
  ...
  when (do_enq && io.enq.bits.addr_crypto_mode) {
    printf(p"[FTQ][ENQ] idx=${enq_ptr} pc=0x${Hexadecimal(io.enq.bits.pc)} cpc_v=${io.enq.bits.cpc_valid.asUInt} cpc=0x${Hexadecimal(io.enq.bits.cpc)} cfi_v=${io.enq.bits.cfi_idx.valid.asUInt} cfi_idx=${io.enq.bits.cfi_idx.bits} call=${io.enq.bits.cfi_is_call.asUInt} ret=${io.enq.bits.cfi_is_ret.asUInt} ras_idx=${io.enq.bits.ghist.ras_idx} ras_top=0x${Hexadecimal(io.enq.bits.ras_top)} ras_top_cpc_v=${io.enq.bits.ras_top_cpc_valid.asUInt} ras_top_cpc=0x${Hexadecimal(io.enq.bits.ras_top_cpc)}\n")
  }
  ...
  val ras_update_cpc = WireInit(0.U(vaddrBitsExtended.W))
  val ras_update_cpc_valid = WireInit(false.B)
  ...
  io.ras_update_cpc := RegNext(ras_update_cpc)
  io.ras_update_cpc_valid := RegNext(ras_update_cpc_valid)
  ...
  val bpd_cpc   = RegNext(cpcs(bpd_idx))
  val bpd_cpc_valid = RegNext(cpc_valids(bpd_idx))
  ...
  when (RegNext(do_commit_update || do_repair_update || do_mispredict_update)) {
    ...
    io.bpdupdate.bits.cpc     := bpd_cpc
    io.bpdupdate.bits.cpc_valid := bpd_cpc_valid
    io.bpdupdate.bits.addr_crypto_mode := bpd_entry.addr_crypto_mode
  ...
  when (io.redirect.valid) {
    ...
    ras_update_cpc := redirect_entry.ras_top_cpc
    ras_update_cpc_valid := redirect_entry.ras_top_cpc_valid
  ...
  when (io.redirect.valid && redirect_entry.addr_crypto_mode) {
    printf(p"[FTQ][REDIRECT] idx=${io.redirect.bits} pc=0x${Hexadecimal(pcs(io.redirect.bits))} ras_idx=${redirect_entry.ras_idx} ras_top=0x${Hexadecimal(redirect_entry.ras_top)} ras_top_cpc_v=${redirect_entry.ras_top_cpc_valid.asUInt} ras_top_cpc=0x${Hexadecimal(redirect_entry.ras_top_cpc)} br_misp=${io.brupdate.b2.mispredict.asUInt} taken=${io.brupdate.b2.taken.asUInt}\n")
  }
  when (RegNext(do_commit_update || do_repair_update || do_mispredict_update) && bpd_entry.addr_crypto_mode) {
    printf(p"[FTQ][BPD-UPD] pc=0x${Hexadecimal(bpd_pc)} cpc_v=${bpd_cpc_valid.asUInt} cpc=0x${Hexadecimal(bpd_cpc)} target=0x${Hexadecimal(bpd_target)} mispred=${RegNext(do_mispredict_update).asUInt} repair=${RegNext(do_repair_update).asUInt} commit=${RegNext(do_commit_update).asUInt} cfi_v=${bpd_entry.cfi_idx.valid.asUInt} cfi_idx=${bpd_entry.cfi_idx.bits} ras_idx=${bpd_entry.ras_idx}\n")
  }
