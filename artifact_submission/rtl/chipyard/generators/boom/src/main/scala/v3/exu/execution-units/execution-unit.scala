...
import boom.v3.util.{BoolToChar,ImmGen, IsKilledByBranch, BranchKillableQueue, BoomCoreStringPrefix}
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val io = IO(new Bundle {
    ...
    ////////////////////////
    val alu_addr_req = if (hasMem) new DecoupledIO(new boom.v3.util.addr_req) else null
    // val alu_data_req = if (hasMem) new DecoupledIO(new boom.v3.util.data_req) else null
    val gen_key_req = if (hasAlu) Valid(new boom.v3.lsu.GenKeyReq) else null
    val cus_reg = Input(UInt(3.W))
    val alu_tlb_result = Flipped(Valid(UInt((vaddrBits+1).W)))
    val alu_lookup = ValidIO(UInt((vaddrBits+1).W))
    val log = Input(Bool())
  ...
  if (hasAlu) {
    io.gen_key_req.valid := false.B
    io.gen_key_req.bits := DontCare
  }
  ...
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  ...
  io.alu_lookup.valid := false.B
  io.alu_lookup.bits := DontCare
  ...
  if (hasAlu) {
    ...
    alu.io.cus_reg := DontCare
    alu.io.dprv := DontCare
    alu.io.prv := DontCare
    ...
    io.gen_key_req.valid := alu.io.resp.valid && alu.io.resp.bits.uop.uopc === uop_gen_key
    io.gen_key_req.bits.uop := alu.io.resp.bits.uop
    when (io.gen_key_req.valid) {
      printf("[EXE-GEN-KEY-REQ] rob=%d pc=0x%x\n",
        io.gen_key_req.bits.uop.rob_idx,
        io.gen_key_req.bits.uop.debug_pc)
    }

    ////////////////////////////////////////////////
    //when (alu.io.req.valid === 1.B && alu.io.req.bits.uop.debug_pc(31,16) === 0x8000.U) 
    when (alu.io.req.valid === 1.B && io.log)
    {
        printf("IsALUValid? Req:(%c) PC:0x%x\n", BoolToChar(alu.io.req.valid, 'V'), alu.io.req.bits.uop.debug_pc)

    }
    //when (alu.io.resp.valid === 1.B && alu.io.resp.bits.uop.debug_pc(31,16) === 0x8000.U) 
    when (alu.io.resp.valid === 1.B && io.log)
    {
        printf("IsALUValid? Res:(%c) PC:0x%x\n", BoolToChar(alu.io.resp.valid, 'V'), alu.io.resp.bits.uop.debug_pc)
    }

    // when (alu.io.req.bits.uop.uopc === uop_gen_key)
    // { 
    //   io.gen_key_req.valid := true.B
    //   // io.gen_key_req.bits.tmp := true.B
    //   io.gen_key_req.bits.uop := alu.io.req.bits.uop
    //   printf("uop_gen_key in exe unit log in execution-unit\n")
    // }.otherwise{
    //   io.gen_key_req.valid := false.B
    //   io.gen_key_req.bits := DontCare
    // }
    ...
    for (i <- 0 until numBypassStages) {
     // when (alu.io.bypass(i).bits.uop.debug_pc(31,16) === 0x8000.U)
      when (io.log)
      {
  ...
  if (hasMul) {
    ...
    //when (imul.io.req.valid === 1.B && imul.io.req.bits.uop.debug_pc(31,16) === 0x8000.U)
    when (imul.io.req.valid === 1.B && io.log)
    {
        printf("IsMulValid? Req:(%c) PC:0x%x\n", BoolToChar(imul.io.req.valid, 'V'), imul.io.req.bits.uop.debug_pc)

    }
    //when (imul.io.resp.valid === 1.B && imul.io.resp.bits.uop.debug_pc(31,16) === 0x8000.U )
    when (imul.io.resp.valid === 1.B && io.log)
    {
  ...
  if (hasIfpu) {
    ...
    ifpu.io.cus_reg := DontCare
    ifpu.io.dprv := DontCare
    ifpu.io.prv  := DontCare
    ifpu.io.req        <> io.req
  ...
  if (hasDiv) {
    ...
    //when (div.io.req.valid === 1.B && div.io.req.bits.uop.debug_pc(31,16) === 0x8000.U)
    when (div.io.req.valid === 1.B && io.log) 
    {
        printf("IsDivValid? Req:(%c) PC:0x%x\n", BoolToChar(div.io.req.valid, 'V'), div.io.req.bits.uop.debug_pc)

    }
    //when (div.io.resp.valid === 1.B && div.io.resp.bits.uop.debug_pc(31,16) === 0x8000.U) 
    when (div.io.resp.valid === 1.B && io.log)
    {
  ...
  if (hasMem) {
    ...
    ////////////////////////////
    io.alu_addr_req <> maddrcalc.io.alu_addr_req
    // io.alu_data_req <> maddrcalc.io.alu_data_req
    io.alu_lookup <> maddrcalc.io.alu_lookup
    io.alu_tlb_result <> maddrcalc.io.alu_tlb_result
    maddrcalc.io.cus_reg := io.cus_reg 
    maddrcalc.io.dprv := io.dprv
    maddrcalc.io.prv  := io.prv
    ////////////////////////////

    //when (maddrcalc.io.req.valid === 1.B && maddrcalc.io.req.bits.uop.debug_pc(31,16) === 0x8000.U) 
    when(maddrcalc.io.req.valid === 1.B && io.log)
    {
        printf("IsMemValid? Req:(%c) PC:0x%x\n", BoolToChar(maddrcalc.io.req.valid, 'V'), maddrcalc.io.req.bits.uop.debug_pc)
    }  
    
    //when (maddrcalc.io.resp.valid === 1.B && maddrcalc.io.resp.bits.uop.debug_pc(31,16) === 0x8000.U ) 
    when (maddrcalc.io.resp.valid === 1.B && io.log)
    {
  ...
  if (writesIrf) {
    val iresp_is_gen_key = if (hasAlu) {
      alu.io.resp.valid && alu.io.resp.bits.uop.uopc === uop_gen_key
    } else {
      false.B
    }
    io.iresp.valid     := iresp_fu_units.map(_.io.resp.valid).reduce(_|_) && !iresp_is_gen_key
    ...
    hasFpiu = hasFpiu) with tile.HasFPUParameters
{
  ...
  if (hasFpu) {
    ...
    fpu.io.cus_reg := DontCare
    fpu.io.dprv := DontCare
    fpu.io.prv  := DontCare
  ...
  if (hasFdiv) {
    ...
    fdivsqrt.io.cus_reg := DontCare
    fdivsqrt.io.dprv := DontCare
    fdivsqrt.io.prv  := DontCare
