  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  val io = IO(new Bundle {
    ...
    ////////////////////////////
    val alu_addr_req = new DecoupledIO(new boom.v3.util.addr_req)
    val alu_lookup = Valid(UInt(coreMaxAddrBits.W))
    val alu_tlb_result = Flipped(Valid(UInt(coreMaxAddrBits.W)))
    val prv = Output(UInt(PRV.SZ.W))
    // val xor_key = Vec(numIntPhysRegs,Flipped(DecoupledIO(UInt(64.W))))
    val gen_xor_req = Output(Bool())
    val gen_xor_resp = Input(Bool())
    val send_xor_req = Output(Bool())
    val log = Output(Bool())
    // val finish = Input(Bool())
    val cus_reg = Output(UInt(3.W))
    val cache_crypto_load_enable_value = Output(Bool())
    val cache_crypto_store_enable_value = Output(Bool())
    val cache_crypto_enable_wen = Output(Bool())
    val icache_crypto_enable_value = Output(Bool())
    val cache_crypto_counter_base_value = Output(UInt(64.W))
    val cache_crypto_counter_base_wen = Output(Bool())
    val dprv = Output(UInt(PRV.SZ.W))
    val c4_flush = Output(Bool())
    ////////////////////////////
  })
  ...
  io.ptw := DontCare
  io.ifu := DontCare
  io.ifu.redirect_addr_crypto_mode := false.B
  ...
  val rob              = Module(new Rob(
                           ...
                           numFpWakeupPorts))
  ///////////////////////////
  // rob.io.finish := io.finish
  io.gen_xor_req := rob.io.gen_xor_req
  rob.io.gen_xor_resp := io.gen_xor_resp  
  io.send_xor_req := rob.io.send_xor_req
  ///////////////////////////
...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  rob.io.next_prv := csr.io.next_prv
  rob.io.prv := csr.io.status.prv

  io.c4_flush := csr.io.c4_flush
  ...
  io.cus_reg := Cat(
    custom_csrs.fetchAddrCryptoEnableCurrent,
    custom_csrs.addrCryptoStoreEnableCurrent,
    custom_csrs.addrCryptoLoadEnableCurrent)
  io.cache_crypto_load_enable_value := custom_csrs.cacheCryptoLoadEnableCurrent
  io.cache_crypto_store_enable_value := custom_csrs.cacheCryptoStoreEnableCurrent
  io.cache_crypto_enable_wen := custom_csrs.cacheCryptoEnableCSRWen
  io.icache_crypto_enable_value := custom_csrs.iCacheCryptoEnableCurrent
  val prevICacheCryptoEnable = RegNext(io.icache_crypto_enable_value, false.B)
  // when (io.icache_crypto_enable_value =/= prevICacheCryptoEnable) {
  //   printf("[ICACHE-CRYPTO-ENABLE] old=%d new=%d cus_reg=0x%x\n",
  //     prevICacheCryptoEnable,
  //     io.icache_crypto_enable_value,
  //     io.cus_reg)
  // }
  val prevPrv = RegNext(csr.io.status.prv, PRV.M.U)
  val prevNextPrv = RegNext(csr.io.next_prv, PRV.M.U)
  val startupTraceLo = "h800001c0".U(xLen.W)
  val startupTraceHi = "h80000640".U(xLen.W)
  def inStartupTraceWindow(pc: UInt): Bool = pc >= startupTraceLo && pc < startupTraceHi
  io.cache_crypto_counter_base_value := custom_csrs.cacheCryptoCounterBaseCurrent
  io.cache_crypto_counter_base_wen := custom_csrs.cacheCryptoCounterBaseCSRWen

  // when (io.log)
  // {
  // printf("io.cus_reg : 0x%x\n",io.cus_reg)
  // }
  ...
  when (RegNext(rob.io.flush.valid)) {
    ...
    val flushUsesEretPrv = RegNext(rob.io.flush.bits.uses_eret_prv)
    // Use a trap-specific sideband here. csr.io.next_prv is the xRET return
    // privilege (mstatus.mpp), not the privilege of the trap handler entry.
    val trapTargetPrv = csr.io.trap_target_prv
    val eretTargetPrv = RegNext(RegNext(csr.io.next_prv))
    val redirectTargetPrv = WireDefault(csr.io.status.prv)
    ...
    when (FlushTypes.useCsrEvec(flush_typ)) {
      ...
      redirectTargetPrv := Mux(flushUsesEretPrv, eretTargetPrv, trapTargetPrv)
    } .otherwise {
    ...
    io.ifu.redirect_addr_crypto_mode := io.cus_reg(2) && (redirectTargetPrv === PRV.U.U)
    ...
    io.ifu.redirect_flush   := true.B
    io.ifu.redirect_addr_crypto_mode := io.cus_reg(2) && (csr.io.status.prv === PRV.U.U)
  ...
  val benchHotStartPc = "h800002a8".U(vaddrBitsExtended.W)
  val benchHotEndPc   = "h8000038a".U(vaddrBitsExtended.W)
  val pureLoopStartPc = "h80000384".U(vaddrBitsExtended.W)
  val pureLoopEndPc   = "h8000038a".U(vaddrBitsExtended.W)
  val exitLoopPc      = "h80000492".U(vaddrBitsExtended.W)

  val decBenchBlocked = RegInit(0.U(32.W))
  val decBenchDisNotReady = RegInit(0.U(32.W))
  val decBenchRollback = RegInit(0.U(32.W))
  val decBenchXcpt = RegInit(0.U(32.W))
  val decBenchBranchMask = RegInit(0.U(32.W))
  val decBenchB1Mispredict = RegInit(0.U(32.W))
  val decBenchB2Mispredict = RegInit(0.U(32.W))
  val decBenchRedirectFlush = RegInit(0.U(32.W))
  val decBenchFire = RegInit(0.U(32.W))

  val decPureBlocked = RegInit(0.U(32.W))
  val decPureDisNotReady = RegInit(0.U(32.W))
  val decPureRollback = RegInit(0.U(32.W))
  val decPureXcpt = RegInit(0.U(32.W))
  val decPureBranchMask = RegInit(0.U(32.W))
  val decPureB1Mispredict = RegInit(0.U(32.W))
  val decPureB2Mispredict = RegInit(0.U(32.W))
  val decPureRedirectFlush = RegInit(0.U(32.W))
  val decPureFire = RegInit(0.U(32.W))
  ...
  //////////////////////////////////
  // iregfile.io.xor_key   <> io.xor_key
  // rob.io.prv := csr.io.status.prv
  io.prv := csr.io.status.prv
  // iregfile.io.cpu.
  //////////////////////////////////
  ...
  // io.alu_tlb_result.valid := false.B
  // io.alu_tlb_result.bits  := DontCare
  ...
  for (w <- 0 until exe_units.length) {
    ...
    if (exe_unit.readsIrf) {
      ...
      if (exe_unit.hasMem) {
        ...
        mem_iss_cnt += 1
        ////////////////////////////
        exe_unit.io.alu_addr_req <> io.alu_addr_req
        // exe_unit.io.alu_data_req <> io.alu_data_req
        exe_unit.io.alu_lookup <> io.alu_lookup
        exe_unit.io.alu_tlb_result <> io.alu_tlb_result
        ////////////////////////////
      } else {
  ...
  iregister_read.io.cus_reg := io.cus_reg

  // iregister_read.io.rreg_addr_req  <> io.rreg_addr_req
  // iregister_read.io.lsu_addr_resp <>
...
//   "CSRFile is being written to spuriously.")
  ...
  val gen_key_req_arb = Module(new Arbiter(new boom.v3.lsu.GenKeyReq, exe_units.alu_units.length))
  for ((alu_unit, idx) <- exe_units.alu_units.zipWithIndex) {
    gen_key_req_arb.io.in(idx).valid := alu_unit.io.gen_key_req.valid
    gen_key_req_arb.io.in(idx).bits := alu_unit.io.gen_key_req.bits
  }
  val gen_key_req_q = Module(new BranchKillableQueue(new boom.v3.lsu.GenKeyReq, 2, flow = false))
  gen_key_req_q.io.enq <> gen_key_req_arb.io.out
  gen_key_req_q.io.brupdate := brupdate
  gen_key_req_q.io.flush := RegNext(rob.io.flush.valid)
  io.lsu.gen_key_req <> gen_key_req_q.io.deq

  io.dprv := io.ptw.status.dprv

  iss_idx = 0
  var bypass_idx = 0
  for (w <- 0 until exe_units.length) {
    val exe_unit = exe_units(w)
    exe_unit.io.cus_reg := io.cus_reg
    exe_unit.io.dprv := io.ptw.status.dprv
    exe_unit.io.prv  := io.ptw.status.prv
    if (exe_unit.readsIrf) {
  ...
  ll_wbarb.io.in(1) <> io.lsu.gen_key_resp
  for (i <- 1 until memWidth) {
  ...
  if (usingFPU) {
    ...
    // Connect FPIU
    ll_wbarb.io.in(2)        <> fp_pipeline.io.to_int
    // Connect FLDs
    fp_pipeline.io.ll_wports <> exe_units.memory_units.map(_.io.ll_fresp).toSeq
  }
  if (usingRoCC) {
    require(usingFPU)
    ll_wbarb.io.in(3)       <> exe_units.rocc_unit.io.ll_iresp
  }
  ...
  val debugWatchVmExitPc = BigInt("4000181a", 16).U(vaddrBits.W)
  val debugWatchTrapRangeLo = BigInt("800016ce", 16).U(vaddrBits.W)
  val debugWatchTrapRangeHi = BigInt("80001704", 16).U(vaddrBits.W)
  for (w <- 0 until coreWidth) {
    val commitWatchPc = rob.io.commit.uops(w).debug_pc(vaddrBits-1, 0)
    val commitWatchHit =
      rob.io.commit.arch_valids(w) &&
      (commitWatchPc === debugWatchVmExitPc ||
       (commitWatchPc >= debugWatchTrapRangeLo && commitWatchPc <= debugWatchTrapRangeHi))
    when (commitWatchHit) {
      printf(p"[COMMIT-WATCH] cycle=0x${Hexadecimal(debug_tsc_reg)} slot=${w.U} " +
        p"pc=0x${Hexadecimal(Sext(commitWatchPc, xLen))} inst=0x${Hexadecimal(rob.io.commit.uops(w).debug_inst)} " +
        p"ldst=${rob.io.commit.uops(w).ldst} dst_rtype=0x${Hexadecimal(rob.io.commit.uops(w).dst_rtype.asUInt)} " +
        p"wdata=0x${Hexadecimal(rob.io.commit.debug_wdata(w))} is_rvc=${rob.io.commit.uops(w).is_rvc}\n")
    }
  }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  val numFtqWhitespace = (ftqSz/4)+1
  ...
  // when ((  (debug_irt_reg & (0xffffff).U) > 100000.U ) &&  (io.ptw.status.prv === PRV.S.U) )
  // when ((io.ptw.status.dprv === PRV.S.U) && (io.ptw.status.prv === PRV.M.U) && (io.cus_reg(1) === 1.U) && ((debug_irt_reg & (0xffffff).U) > 22500.U ))
  // when(io.ptw.status.prv === PRV.U.U)
  // when (( io.ptw.status.prv === PRV.S.U ) && ((debug_irt_reg & (0xffffff).U) > 36000.U ))
  // {
  ...
  // io.log :=  ((io.ptw.status.dprv === PRV.S.U) && (io.ptw.status.prv === PRV.M.U) && (io.cus_reg(1) === 1.U) && ((debug_irt_reg & (0xffffff).U) > 22500.U ))
  // io.log := io.ptw.status.prv === PRV.U.U
  // io.log := ( io.ptw.status.prv === PRV.S.U ) && ((debug_irt_reg & (0xffffff).U) > 20000.U )
  // io.log := (io.cus_reg(3) === 1.U) && ( io.ptw.status.prv === PRV.S.U ) && ((debug_irt_reg & (0xffffff).U) > 20000.U )
  io.log := false.B
  for (eu <- exe_units)
  {
    eu.io.log := io.log
  ...
  rob.io.log := io.log
  mem_iss_unit.io.log := io.log
  int_iss_unit.io.log := io.log
  fp_pipeline.io.log := io.log
  
  //printf("Decode:\n")
  for (w <- 0 until coreWidth) {
    when (io.log)
    {
      printf("Decode: Slot:%d (PC:0x%x Valids:%c%c Inst:DASM(0x%x)) dec_valid : %c ,dec_stalls : %c \n",
        w.U,
        dec_uops(w).debug_pc,
        BoolToChar(io.ifu.fetchpacket.valid && dec_fbundle.uops(w).valid && !dec_finished_mask(w), 'V'),
        BoolToChar(dec_fire(w), 'V'),
        dec_fbundle.uops(w).bits.debug_inst,BoolToChar(dec_valids(w), 'V') ,BoolToChar(dec_stalls(w), 'V'))
    }
  }
  ...
  //  printf("Rename:\n")
  for (w <- 0 until coreWidth) { 
    //when (rename_stage.io.ren2_uops(w).debug_pc(31,16) === 0x8000.U)
    when (io.log)
    {
      printf("Rename: Slot:%d (PC:0x%x Valid:%c Inst:DASM(0x%x))\n",
        w.U,
        rename_stage.io.ren2_uops(w).debug_pc,
  ...
  when ( io.log )
  {
    ...
    printf("    (State:%c%c%c Rdy:%c LAQFull:%c STQFull:%c Flush:%c BMskFull:%c) BMsk:0x%x Mode:%c\n",
        ...
        dec_brmask_logic.io.debug_branch_mask,
  ...
  */
    ...
    /*
    printf("Instruction Cache Miss:%c, Instruction TLBMiss:%c, L2TLBMiss:%c \n", BoolToChar(io.ifu.perf.acquire, 'T'),
     BoolToChar(io.ifu.perf.tlbMiss, 'T'), BoolToChar(io.ptw.perf.l2miss, 'T'))
    */
    /*
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  val dumpedForwardedPMP = RegInit(false.B)
  when (!dumpedForwardedPMP &&
        io.ptw.ptbr.mode.orR &&
        csr.io.status.prv === PRV.M.U &&
        io.ptw.pmp.take(8).map(_.cfg.a === 0.U).reduce(_||_)) {
    dumpedForwardedPMP := true.B
    for (pmpIndex <- 0 until 8) {
      chisel3.printf("[CORE-PTW-PMP] idx=%d addr=0x%x cfg_l=%d cfg_a=0x%x cfg_r=%d cfg_w=%d cfg_x=%d\n",
        pmpIndex.U,
