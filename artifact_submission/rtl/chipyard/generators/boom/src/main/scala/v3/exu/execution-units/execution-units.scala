...
class ExecutionUnits(val fpu: Boolean)(implicit val p: Parameters) extends HasBoomCoreParameters
{
  ...
  if (!fpu) {
    ...
    for (w <- 0 until int_width) {
      ...
      // alu_exe_unit.io.cus_reg := DontCare
      alu_exe_unit.io.alu_tlb_result.valid := false.B
      alu_exe_unit.io.alu_tlb_result.bits := DontCare
    ...
    for (w <- 0 until fp_width) {
      val fpu_exe_unit = Module(new FPUExeUnit(hasFpu = true,
                                             ...
                                             hasFpiu = (w==0)))
      fpu_exe_unit.io.cus_reg := DontCare
      fpu_exe_unit.io.dprv := DontCare
      fpu_exe_unit.io.prv  := DontCare
      fpu_exe_unit.io.alu_tlb_result.valid := false.B 
      fpu_exe_unit.io.alu_tlb_result.bits := DontCare
