  ...
  with HasBoomUOP
{
  ...
  val origin_addr = UInt((vaddrBits+1).W)// only for maddr -> LSU
  ...
  // val addr_is_ca = Bool() // only for mcalc
}
  ...
  (implicit p: Parameters) extends BoomModule
{
  val io = IO(new Bundle {
    ...
    val alu_addr_req = if (isMemAddrCalcUnit) DecoupledIO(new boom.v3.util.addr_req) else null
    // val alu_data_req = if (isMemAddrCalcUnit) DecoupledIO(new boom.v3.util.data_req) else null
    val alu_lookup = if (isMemAddrCalcUnit) ValidIO(UInt((vaddrBits+1).W)) else null
    val alu_tlb_result = if (isMemAddrCalcUnit) Flipped(ValidIO(UInt((vaddrBits+1).W))) else null
    // val alu_data_req = if (isMemAddrCalcUnit) DecoupledIO(new boom.v3.util.data_req) else null
    // val  = if (isMemAddrCalcUnit) Input(Bool()) else null
  ...
  with freechips.rocketchip.rocket.constants.ScalarOpConstants
{
  ...
  // {
  //   printf("!!!! 0x%x\n",sum)
  //   printf("rs1 : 0x%x\n",io.req.bits.rs1_data.asSInt)
  //   printf("imm : 0x%x\n",io.req.bits.uop.imm_packed(19,8).asSInt)
  // }

  io.alu_addr_req.valid := false.B
  io.alu_addr_req.bits := DontCare

  // io.alu_data_req.valid := false.B
  // io.alu_data_req.bits := DontCare
  ...
  when(asyncLdAddrEnable  && io.req.bits.uop.uses_ldq &&  ( io.req.valid && !IsKilledByBranch(io.brupdate, io.req.bits.uop) ))
  {
    io.alu_lookup.valid := true.B
    io.alu_lookup.bits :=  Cat(ea_sign, sum(vaddrBits-1,0)).asUInt
    // printf("load vaddrBits : %d\n",vaddrBits.U);
    // 如果 当时 alu_tlb无法处理,直接转发
    // printf("load memaddrcalc : 0x%x\n",effective_address)
    when(!io.alu_tlb_result.valid)
    {
      io.alu_addr_req.valid := true.B
      io.alu_addr_req.bits.addr := effective_address
      io.alu_addr_req.bits.uop := io.req.bits.uop
      io.alu_addr_req.bits.data := 0.U
      io.alu_addr_req.bits.data_valid := false.B
      io.resp.valid := false.B
    }.otherwise
    {
      printf("load alu tlb hit\n");
      effective_address := io.alu_tlb_result.bits
      io.resp.bits.origin_addr := Cat(ea_sign, sum(vaddrBits-1,0)).asUInt
    }
  }.elsewhen(asyncStAddrEnable &&
             io.req.bits.uop.uses_stq &&
             io.req.bits.uop.ctrl.is_sta &&
             (io.req.valid && !IsKilledByBranch(io.brupdate, io.req.bits.uop)))
  {
    io.alu_lookup.valid := true.B
    io.alu_lookup.bits :=  Cat(ea_sign, sum(vaddrBits-1,0)).asUInt
    // printf("store vaddrBits : %d\n",vaddrBits.U);
    // 如果 当时 alu_tlb无法处理,直接转发
    // printf("store memaddrcalc : 0x%x\n",effective_address)
    ...
    when(!io.alu_tlb_result.valid)
    {
      ...
      io.alu_addr_req.bits.addr := effective_address
      io.alu_addr_req.bits.uop := io.req.bits.uop
      io.alu_addr_req.bits.data := store_data
      io.alu_addr_req.bits.data_valid := io.req.bits.uop.ctrl.is_std
      io.resp.valid := false.B
     
    }.otherwise
    {
      // printf("store tlb hit\n");
      effective_address := io.alu_tlb_result.bits
      io.resp.bits.origin_addr := Cat(ea_sign, sum(vaddrBits-1,0)).asUInt
    }
  }

  // 那如果这个cycle tlb miss 了怎么办
  // when (io.req.bits.uop.ctrl.is_std && (io.cus_reg(1) === 1.U ) && ( io.req.valid && !IsKilledByBranch(io.brupdate, io.req.bits.uop) ) && (Cat(ea_sign, sum(vaddrBits-1,0))(31,0).asUInt =/=  "h80001000".U(32.W)) )
  // {
  //   printf("store data : 0x%x\n",store_data)
  //   io.alu_data_req.bits.data := store_data
  //   io.alu_data_req.bits.uop := io.req.bits.uop
  //   io.alu_data_req.bits.addr := Cat(ea_sign, sum(vaddrBits-1,0)).asUInt 
  //   io.alu_data_req.valid := true.B
  //   io.resp.bits.uop.ctrl.is_std := false.B // 告诉后续阶段这个store data无效
  // }
  ...
  when (io.alu_addr_req.valid) {
    assert(asyncLdAddrEnable || asyncStAddrEnable,
      "[maddrcalc] async addr request fired while cus_reg path is disabled")
    assert(!io.alu_addr_req.bits.uop.uses_stq || io.alu_addr_req.bits.uop.ctrl.is_sta,
      "[maddrcalc] store async addr request must come from STA/STAD, not pure STD")
  }
  if (dataWidth > 63) {
