...
import freechips.rocketchip.rocket.CacheCryptoConfigBundle
...
trait HasTileInputConstants { this: LazyModule with Attachable with InstantiatesHierarchicalElements =>
  ...
  /** tileCacheCryptoConfigNexusNode collects cache-crypto runtime configuration from tiles and forwards
    * it to downstream consumers such as the inclusive L2.
    *
    * Both `enable` and `base` use update-style semantics and therefore carry their own write-enable.
    */
  lazy val tileCacheCryptoConfigNexusNode = LazyModule(new BundleBridgeNexus[CacheCryptoConfigBundle](
    inputFn = { inputs =>
      val out = Wire(new CacheCryptoConfigBundle)
      val anyEnableWen = inputs.map(_.enable_wen).reduceOption(_ || _).getOrElse(false.B)
      val anyBaseWen = inputs.map(_.base_wen).reduceOption(_ || _).getOrElse(false.B)
      out.enable_wen := anyEnableWen
      out.enable := Mux1H(
        inputs.map(_.enable_wen) :+ !anyEnableWen,
        inputs.map(_.enable) :+ false.B)
      out.base_wen := anyBaseWen
      out.base := Mux1H(
        inputs.map(_.base_wen) :+ !anyBaseWen,
        inputs.map(_.base) :+ 0.U(64.W))
      out
    },
    outputFn = (cfg: CacheCryptoConfigBundle, n: Int) => Seq.fill(n)(cfg),
    default = Some(() => 0.U.asTypeOf(new CacheCryptoConfigBundle)),
...
trait CanAttachTile {
  ...
  def connectInputConstants(domain: TilePRCIDomain[TileType], context: TileContextType): Unit = {
    ...
    context.tileCacheCryptoConfigNexusNode := domain.element.cacheCryptoConfigSourceNode
