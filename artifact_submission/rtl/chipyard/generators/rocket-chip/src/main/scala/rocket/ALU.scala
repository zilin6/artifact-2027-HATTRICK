...
object ALU {
  ...
  // 其他三条指令先复用 ADD
  def FN_GEN_KEY = 20.U
  // def FN_CC_ISA_ENCPTR = 21.U
  def FN_LOAD_KEY = 22.U
  def FN_STORE_KEY = 23.U
  ///////////////////
...
abstract class AbstractALU(implicit p: Parameters) extends CoreModule()(p) {
  val io = IO(new Bundle {
    ...
    ////////////////////
    //val cc_isa_encptr_out = Decoupled(new pointer_encptr_req)
    ////////////////////
  })
}
...
class ALU(implicit p: Parameters) extends AbstractALU()(p) {
  ...
  ////////////////
  // 定义  cc_isa_encptr ,in_1 ,in_2  
  //但是要经过pointer engine
  // 先放到外面吧，以后再接进来
  /*
  when (io.fn === FN_CC_ISA_ENCPTR)
  {
    // in_2位64位，前32位为 power,后32位为version
    val power := in_2[63,32]
    val version := in_2[31,0]
    val plaintext := io.in1[53,0] +  version << 53
    val tweak := io.in2 
    printf("cc_isa_encptr")
    printf("in_1 : 0x%x\n",io.in_1)
    printf("in_2 : 0x%x\n",io.in_2)
    printf("power : 0x%x\n",power)
    printf("version : 0x%x\n",version)
