...
class MIP(implicit p: Parameters) extends CoreBundle()(p) with HasCoreParameters {
...
class PerfCounterIO(implicit p: Parameters) extends CoreBundle with HasCoreParameters {
...
class CSRFile(
  ...
  val io = IO(new CSRFileIO(hasBeu) {
    ...
    ////////////////
    val next_prv = Output(UInt(2.W))
    val trap_target_prv = Output(UInt(2.W))
  ...
  val reg_dprv = RegInit(PRV.M.U(2.W))
  val reg_trap_target_prv = RegInit(PRV.M.U(2.W))

  val cur_dprv = io.status.dprv
  val dprv_changed = cur_dprv =/= reg_dprv

  // 下一个周期记住当前 dprv
  reg_dprv := cur_dprv

  // 当 dprv 变化时，flush 拉高一个周期，其余为 0
  io.c4_flush := dprv_changed
  ...
  /////////////////
  io.status.mprv := reg_mstatus.mprv 
  ////////////////////=/ 
  ...
  val debugWatchTrapPcEcall = BigInt("4000181a", 16).U(xLen.W)
  val debugWatchTrapLineBase = BigInt("80001700", 16).U(xLen.W)
  val debugWatchTrapLine = (epc >> 6) === (debugWatchTrapLineBase >> 6)
  val debugWatchUserBounceLo = BigInt("40000000", 16).U(xLen.W)
  val debugWatchUserBounceHi = BigInt("40008000", 16).U(xLen.W)
  val debugWatchTrapHandlerLo = BigInt("80000540", 16).U(xLen.W)
  val debugWatchTrapHandlerHi = BigInt("80000580", 16).U(xLen.W)
  val debugWatchBodyLo = BigInt("40001700", 16).U(xLen.W)
  val debugWatchBodyHi = BigInt("40001a20", 16).U(xLen.W)
  val debugWatchTrapWideLo = BigInt("80000c80", 16).U(xLen.W)
  val debugWatchTrapWideHi = BigInt("80001450", 16).U(xLen.W)
  val debugWatchTrap =
    exception && (epc === debugWatchTrapPcEcall || debugWatchTrapLine ||
      cause === 2.U || cause === 9.U ||
      (epc >= debugWatchUserBounceLo && epc < debugWatchUserBounceHi) ||
      (epc >= debugWatchTrapHandlerLo && epc < debugWatchTrapHandlerHi) ||
      (epc >= debugWatchBodyLo && epc < debugWatchBodyHi) ||
      (epc >= debugWatchTrapWideLo && epc < debugWatchTrapWideHi))

  when (exception) {
    when (debugWatchTrap) {
      printf(p"[CSR-TRAP] epc=0x${Hexadecimal(epc)} cause=0x${Hexadecimal(cause)} tval=0x${Hexadecimal(tval)} " +
        p"prv=${reg_mstatus.prv} delegate=${delegate} delegateVS=${delegateVS} " +
        p"trapToDebug=${trapToDebug} trapToNmi=${trapToNmi} status_mprv=${reg_mstatus.mprv} " +
        p"trap_target_prv_reg=${reg_trap_target_prv}\n")
    }
    when (trapToDebug) {
      when (!reg_debug) {
  ...
  when (insn_ret) {
    ...
    val ret_path = WireInit("h0".U(3.W))
    ...
    when ((io.evec >= debugWatchBodyLo && io.evec < debugWatchBodyHi) ||
          (io.evec >= debugWatchTrapWideLo && io.evec < debugWatchTrapWideHi)) {
      printf(p"[CSR-XRET] evec=0x${Hexadecimal(io.evec)} ret_prv=${ret_prv} ret_path=${ret_path} " +
        p"status_prv=${reg_mstatus.prv} next_prv=${new_prv} mprv=${reg_mstatus.mprv} " +
        p"mepc=0x${Hexadecimal(reg_mepc)} sepc=0x${Hexadecimal(reg_sepc)} dpc=0x${Hexadecimal(reg_dpc)}\n")
    }
  }

  ///////////////////////////
  io.next_prv := legalizePrivilege(reg_mstatus.mpp)
  io.trap_target_prv := reg_trap_target_prv
  ///////////////////////////
  ...
  for (((customIO, reg), csr) <- (io.customCSRs zip reg_custom).zip(customCSRs)) {
    val trackCSR = csr.id == 0x3f2 || csr.id == 0x3f4
    val prevReg = RegNext(reg, 0.U(xLen.W))
  }
    // printf("only one custom reg\n")
  ...
  when (csr_wen) {
      ...
      when (decoded_addr(CSRs.satp)) {
        if (usingVM) {
          ...
          when (new_satp.mode.isOneOf(satp_valid_modes.map(_.U))) {
            chisel3.printf("[CSR-SATP-WR] cycle=0x%x pc=0x%x prv=%d cmd=0x%x old_mode=0x%x old_ppn=0x%x new_mode=0x%x new_ppn=0x%x raw=0x%x\n",
              reg_cycle,
              io.pc,
              reg_mstatus.prv,
              io.rw.cmd,
              reg_satp.mode,
              reg_satp.ppn,
              (new_satp.mode & satp_valid_modes.reduce(_|_).U),
              new_satp.ppn(ppnBits-1,0),
              wdata)
    ...
    if (reg_pmp.nonEmpty) for (((pmp, next), i) <- (reg_pmp zip (reg_pmp.tail :+ reg_pmp.last)).zipWithIndex) {
      ...
      when (decoded_addr(CSRs.pmpcfg0 + pmpCfgIndex(i)) && !pmp.cfgLocked) {
        ...
        if (i < 8) {
          val effW = newCfg.w && newCfg.r
          val effA = if (pmpGranularity.log2 > PMP.lgAlign) Cat(newCfg.a(1), newCfg.a.orR) else newCfg.a
          chisel3.printf("[CSR-PMP-CFG-WR] cycle=0x%x idx=%d wdata=0x%x cfg_l=%d cfg_a=0x%x cfg_r=%d cfg_w=%d cfg_x=%d eff_a=0x%x eff_w=%d\n",
            reg_cycle, i.U, wdata, newCfg.l, newCfg.a, newCfg.r, newCfg.w, newCfg.x, effA, effW)
        }
      }
      when (decoded_addr(CSRs.pmpaddr0 + i) && !pmp.addrLocked(next)) {
        pmp.addr := wdata
        if (i < 8) {
          chisel3.printf("[CSR-PMP-ADDR-WR] cycle=0x%x idx=%d wdata=0x%x\n",
            reg_cycle, i.U, wdata)
        }
      }
    }
  def writeCustomCSR(customIO: CustomCSRIO, csr: CustomCSR, reg: UInt) = {
    val mask = csr.mask.U(xLen.W)
    when (decoded_addr(csr.id)) {
      val newValue = (wdata & mask) | (reg & ~mask)
      reg := newValue
      customIO.wen := true.B
      if (csr.id == 0x3f0 || csr.id == 0x3f1 || csr.id == 0x3f3) {
        printf(p"[CSR-CUSTOM-WR] cycle=0x${Hexadecimal(reg_cycle)} pc=0x${Hexadecimal(io.pc)} " +
          p"prv=${reg_mstatus.prv} csr=0x${Hexadecimal(csr.id.U)} cmd=0x${Hexadecimal(io.rw.cmd)} " +
          p"old=0x${Hexadecimal(reg)} new=0x${Hexadecimal(newValue)}\n")
      }
    }
  }
