...
import freechips.rocketchip.util.BoolToChar4Rocket
...
class TLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  when (watchPtwPhysReq) {
    chisel3.printf("[TLB-PTW-CHECK] req_addr=0x%x mpu_paddr=0x%x page=0x%x prv=%d cmd=0x%x size=%d vm_enabled=%d mpu_priv=%d pmp_r=%d pmp_w=%d pmp_x=%d pma_r=%d pma_w=%d pma_x=%d cacheable=%d deny_debug=%d prot_r=%d prot_w=%d prot_x=%d\n",
      io.req.bits.vaddr,
      mpu_physaddr,
      mpu_physaddr(paddrBits-1, pgIdxBits),
      io.req.bits.prv,
      io.req.bits.cmd,
      io.req.bits.size,
      vm_enabled,
  ...
  when (do_refill) {
    ...
    when (special_entry.nonEmpty.B && !io.ptw.resp.bits.homogeneous) {
      ...
      // when (io.log) {
        printf("=== TLB special_entry refill ===\n")
        printf("  r_refill_tag       = 0x%x\n", r_refill_tag)
        printf("  level              = %d\n", io.ptw.resp.bits.level)
        printf("  refill_v           = %d, invalidate_refill = %d\n",
          refill_v, invalidate_refill)
        // 如果有保存原始 VA，比如 r_refill_vaddr，可以一起打
        // printf("  refill_vaddr       = 0x%x\n", r_refill_vaddr)

        printf("  newEntry.ppn       = 0x%x\n", newEntry.ppn)
        printf("  newEntry perm: u=%d g=%d sr=%d sw=%d sx=%d\n",
          newEntry.u, newEntry.g,
          newEntry.sr, newEntry.sw, newEntry.sx)
      ...
      for ((e, i) <- superpage_entries.zipWithIndex) when (r_superpage_repl_addr === i.U) {
        printf("=== TLB superpage refill ===\n")
        printf("  waddr(way)         = %d\n", waddr)
        printf("  r_superpage_hit.v  = %d, hit_way = %d\n",
          r_superpage_hit.valid, r_superpage_hit.bits)
        printf("  r_superpage_repl   = %d\n", r_superpage_repl_addr)
        printf("  level              = %d\n", io.ptw.resp.bits.level)
        printf("  r_refill_tag       = 0x%x\n", r_refill_tag)
        printf("  refill_v           = %d, invalidate_refill = %d\n",
          refill_v, invalidate_refill)
    ...
    // refill sectored_hit
      ...
      for ((e, i) <- sectored_entries(r_memIdx).zipWithIndex) when (waddr === i.U) {
        printf("=== TLB sectored refill ===\n")
        printf("  r_memIdx(set)      = %d\n", r_memIdx)           // set index
        printf("  waddr(way)         = %d\n", waddr)             // 目标 way
        printf("  loop i             = %d\n", i.U)               // 当前 for 中的 i
        printf("  r_sectored_hit.vld = %d, hit_way = %d\n",
          r_sectored_hit.valid, r_sectored_hit.bits)
        printf("  r_sectored_repl_way= %d\n", r_sectored_repl_addr)

        // 本次 refill 的 tag / valid / 其他控制信号
        printf("  r_refill_tag       = 0x%x\n", r_refill_tag)
        printf("  refill_v           = %d, invalidate_refill = %d\n",
          refill_v, invalidate_refill)

        // 如果你有保存原始 VA 的寄存器，例如 r_refill_vaddr，就一起打出来
        // printf("  refill_vaddr       = 0x%x\n", r_refill_vaddr)

        // newEntry 里的 PPN 和权限位（字段名按你实际类型改）
        printf("  newEntry.ppn       = 0x%x\n", newEntry.ppn)
        printf("  newEntry perm: u=%d g=%d sr=%d sw=%d sx=%d\n",
          newEntry.u, newEntry.g,
          newEntry.sr, newEntry.sw, newEntry.sx,
  ...
  when (watchPtwPhysReq) {
    chisel3.printf("[TLB-PTW-RESP] paddr=0x%x miss=%d ae_ld=%d pf_ld=%d ma_ld=%d ae_st=%d pf_st=%d hit=0x%x ae_ld_array=0x%x pr_array=0x%x ae_array=0x%x\n",
      Cat(ppn, io.req.bits.vaddr(pgIdxBits-1, 0)),
      io.resp.miss,
      io.resp.ae.ld,
      io.resp.pf.ld,
      io.resp.ma.ld,
