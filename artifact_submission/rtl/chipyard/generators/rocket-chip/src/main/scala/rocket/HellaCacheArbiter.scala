...
class HellaCacheArbiter(n: Int)(implicit p: Parameters) extends Module
{
  ...
  if (n == 1) {
    ...
    when (io.mem.req.fire && io.mem.req.bits.phys) {
      chisel3.printf("[HCARB-PHYS-REQ] req_id=%d addr=0x%x cmd=0x%x size=%d dprv=%d dv=%d tag=0x%x s1_kill=%d s2_kill=%d\n",
        io.mem.req.bits.tag(log2Up(n)-1, 0),
