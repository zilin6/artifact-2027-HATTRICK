...
class PTE(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  def s_mode_pf(dummy: Int = 0) = reserved_for_software(0)
...
class PTW(n: Int)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val probeReqActive = RegInit(false.B)
  val probeReqVpn = Reg(UInt(vpnBits.W))
  val probeReqDest = Reg(UInt(log2Ceil(n).W))
  ...
  when(mem_resp_valid && io.log)
  {
    printf("PTW MEM_REQ: state=%d addr=0x%x vpn=0x%x count=%d aux_count=%d stage2=%d do_both=%d \n",
    state,
  ...
  val (pte, invalid_paddr, invalid_gpa) = {
    ...
    res.reserved_for_software := tmp.reserved_for_software
  ...
  val vpn_idxs_log = (0 until pgLevels).map { i =>
      val width = pgLevelBits + (if (i <= pgLevels - minPgLevels) hypervisorExtraAddrBits else 0)
      (vpn >> (pgLevels - i - 1) * pgLevelBits)(width - 1, 0)
    }
  when (io.log && (state === s_req || state === s_dummy1))
  {
  }
  ...
  val (l2_hit, l2_error, l2_pte, l2_tlb_ram) = if (coreParams.nL2TLBEntries == 0) (false.B, false.B, WireDefault(0.U.asTypeOf(new PTE)), None) else {
    ...
    when (l2_refill && !invalidated) {
      ...
      entry.w := r_pte.w
      entry.r := r_pte.r
      entry.s_mode_pf := r_pte.s_mode_pf()
      entry.tag := r_tag
    ...
    when (probeReqActive && r_req_dest === probeReqDest && r_req.addr === probeReqVpn && s0_valid) {
      chisel3.printf("[PTW-L2-LOOKUP-S0] vpn=0x%x tag=0x%x idx=0x%x valid_vec=0x%x state=%d\n",
        arb.io.out.bits.bits.addr,
        r_tag,
        r_idx,
        r_valid_vec,
        state)
    }
    when (probeReqActive && r_req_dest === probeReqDest && r_req.addr === probeReqVpn && s2_valid) {
      chisel3.printf("[PTW-L2-LOOKUP-S2] vpn=0x%x tag=0x%x idx=0x%x valid_vec=0x%x hit_vec=0x%x hit=%d way0_tag=0x%x way0_ppn=0x%x way0_r=%d way0_w=%d way0_x=%d way0_u=%d way0_a=%d way0_d=%d\n",
        r_req.addr,
        r_tag,
        r_idx,
        s2_valid_vec,
        VecInit(s2_hit_vec).asUInt,
        s2_hit,
        s2_entry_vec.head.tag,
        s2_entry_vec.head.ppn,
        s2_entry_vec.head.r,
        s2_entry_vec.head.w,
        s2_entry_vec.head.x,
        s2_entry_vec.head.u,
    ...
    s2_pte.reserved_for_software := Cat(0.U(1.W), s2_hit_entry.s_mode_pf)
  ...
  for (i <- 0 until io.requestor.size) {
    ...
    when (io.log && resp_valid.orR && i.U === 0.U) {
        val dest = OHToUInt(resp_valid.asUInt)
        printf("PTW RESP: dest=%d vpn=0x%x level=%d homogeneous=%d pmaHom=%d pmpHom=%d ppn=0x%x v=%d r=%d w=%d x=%d u=%d a=%d d=%d pf=%d gf=%d ae_ptw=%d ae_final=%d\n",
          dest,
          r_req.addr,
          max_count,
          (homogeneous || pageGranularityPMPs.B),
          pmaHomogeneous,
          pmpHomogeneous,
          r_pte.ppn,
          r_pte.v,
        ...
        when (probeReqActive && dest === probeReqDest && r_req.addr === probeReqVpn) {
          chisel3.printf("[PTW-PROBE-RESP] dest=%d vpn=0x%x level=%d ppn=0x%x v=%d r=%d w=%d x=%d u=%d a=%d d=%d pf=%d gf=%d ae_ptw=%d ae_final=%d stage2=%d stage2_final=%d count=%d aux_count=%d\n",
            dest,
            r_req.addr,
            max_count,
            r_pte.ppn,
  ...
  switch (state) {
    is (s_ready) {
      when (arb.io.out.fire) {
        ...
        {
          printf(
            "PTW REQ: dest=%d vpn=0x%x vstage1=%d stage2=%d need_gpa=%d satp_ppn=0x%x vsatp_ppn=0x%x hgatp_ppn=0x%x\n",
            arb.io.chosen,
            arb.io.out.bits.bits.addr,
            arb.io.out.bits.bits.vstage1,
        ...
        when (io.dpath.status.mprv && io.dpath.status.dprv === PRV.U.U && !arb.io.out.bits.bits.vstage1 && !arb.io.out.bits.bits.stage2) {
          probeReqActive := true.B
          probeReqVpn := arb.io.out.bits.bits.addr
          probeReqDest := arb.io.chosen
          chisel3.printf("[PTW-U2-REQ] dest=%d vpn=0x%x satp_ppn=0x%x mprv=%d dprv=%d prv=%d state=%d\n",
            arb.io.chosen,
            arb.io.out.bits.bits.addr,
            io.dpath.ptbr.ppn,
            io.dpath.status.mprv,
            io.dpath.status.dprv,
            io.dpath.status.prv,
            state)
          chisel3.printf("[PTW-PROBE-REQ] dest=%d vpn=0x%x satp_ppn=0x%x mprv=%d dprv=%d prv=%d\n",
            arb.io.chosen,
            arb.io.out.bits.bits.addr,
            io.dpath.ptbr.ppn,
            io.dpath.status.mprv,
    ...
    is (s_wait2) {
      ...
      when (io.mem.s2_xcpt.ae.ld) {
        chisel3.printf("[PTW-U2-AE] dest=%d vpn=0x%x state=%d count=%d aux_count=%d pte_addr=0x%x mprv=%d dprv=%d prv=%d mem_ae=%d resp_ae_ptw=%d\n",
          r_req_dest,
          r_req.addr,
          state,
          count,
          ...
          resp_ae_ptw)
        when (probeReqActive && r_req_dest === probeReqDest && r_req.addr === probeReqVpn) {
          chisel3.printf("[PTW-AE-PTW] vpn=0x%x state=%d count=%d aux_count=%d pte_addr=0x%x seeded_ppn=0x%x seeded_v=%d seeded_r=%d seeded_w=%d seeded_x=%d seeded_u=%d\n",
            r_req.addr,
            state,
            count,
            aux_count,
  ...
  when (l2_hit && !l2_error && !resp_gf) {
    ...
    when (probeReqActive && r_req_dest === probeReqDest && r_req.addr === probeReqVpn) {
      chisel3.printf("[PTW-L2-RESP] vpn=0x%x state=%d count=%d ppn=0x%x v=%d r=%d w=%d x=%d u=%d a=%d d=%d\n",
        r_req.addr,
        state,
        count,
        l2_pte.ppn,
  ...
  when (mem_resp_valid) {
    ...
    when (probeReqActive && r_req_dest === probeReqDest && r_req.addr === probeReqVpn) {
      chisel3.printf("[PTW-PROBE-MEM] state=%d vpn=0x%x pte_addr=0x%x raw=0x%x count=%d aux_count=%d traverse=%d pte_ppn=0x%x pte_v=%d pte_r=%d pte_w=%d pte_x=%d pte_u=%d pte_a=%d pte_d=%d invalid_paddr=%d invalid_gpa=%d\n",
        state,
        r_req.addr,
        pte_addr,
        mem_resp_data,
    ...
    when (probeReqActive && !traverse) {
      probeReqActive := false.B
    }
  }
  when (io.mem.s2_nack) {
