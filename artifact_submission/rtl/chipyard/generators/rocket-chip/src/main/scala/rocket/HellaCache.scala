...
abstract class HellaCache(tileId: Int)(implicit p: Parameters) extends LazyModule
  ...
  val node = TLClientNode(Seq(TLMasterPortParameters.v1(
    ...
    minLatency = 1,
    requestFields = tileParams.core.useVM.option(Seq()).getOrElse(Seq(AMBAProtField())) ++ Seq(
      CacheCryptoWritebackMetaField(
        nonceBits = p(CacheCryptoNonceBitsKey),
...
trait HasHellaCacheModule {
  ...
  // outer.dcache.module.io.cacheCryptoEnable := p(CacheCryptoEnableKey).B
  outer.dcache.module.io.cacheCryptoEnable := outer.dcache.module.io.ptw.customCSRs.cacheCryptoEnableCSR
}
...
class L1Metadata(implicit p: Parameters) extends L1HellaCacheBundle()(p) {
  ...
  val reenc_active = Bool()
  val cryptoLine = Bool()
}

object L1Metadata {
  def apply(tag: Bits, coh: ClientMetadata)(implicit p: Parameters): L1Metadata =
    apply(tag, coh, false.B, false.B)

  def apply(tag: Bits, coh: ClientMetadata, reenc_active: Bool)(implicit p: Parameters): L1Metadata =
    apply(tag, coh, reenc_active, false.B)

  def apply(tag: Bits, coh: ClientMetadata, reenc_active: Bool, cryptoLine: Bool)(implicit p: Parameters): L1Metadata =
    {
      val meta = Wire(new L1Metadata)
      meta.tag := tag
      meta.coh := coh
      meta.reenc_active := reenc_active
      meta.cryptoLine := cryptoLine
      meta
    }
}
