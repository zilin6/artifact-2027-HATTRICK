...
class ICacheModule(outer: ICache) extends LazyModuleImp(outer)
  ...
  when (io.cacheCryptoEnable && refill_fire) {
    printf(p"[ICacheCrypto] refill-req paddr=0x${Hexadecimal(refill_paddr)} idx=${Hexadecimal(refill_idx)} tag=0x${Hexadecimal(refill_tag)}\n")
  }
  ...
  refillCryptoMeta.foreach { f =>
    refillMetaCounter := f.counter
  }
 ...
 * */
  ...
  when (refill_done) {
    ...
    l1NonceMeta(refill_idx)(repl_way) := Mux(io.cacheCryptoEnable, refillMetaNonce, 0.U)
    l1CtrMeta(refill_idx)(repl_way) := Mux(io.cacheCryptoEnable, refillMetaCounter, 0.U)
    l1BusyMeta(refill_idx)(repl_way) := Mux(io.cacheCryptoEnable, refillMetaBusy, false.B)
    when (io.cacheCryptoEnable) {
      printf(p"[ICacheCrypto] refill-meta set=${Hexadecimal(refill_idx)} way=${Decimal(repl_way)} nonce=0x${Hexadecimal(refillMetaNonce)} ctr=${Decimal(refillMetaCounter)} busy=${refillMetaBusy}\n")
    }
  ...
  for (i <- 0 until nWays) {
    ...
    val s1CryptoBusy = l1BusyMeta(s1_idx)(i) && !scratchpadHit
    s1_tag_hit(i) := Mux(io.cacheCryptoEnable, (tagMatch || scratchpadHit) && !s1CryptoBusy, tagMatch || scratchpadHit)
    when (io.cacheCryptoEnable && s1_valid && (tagMatch || scratchpadHit) && s1CryptoBusy) {
      printf(p"[ICacheCrypto] busy-hit-suppressed set=${Hexadecimal(s1_idx)} way=${Decimal(i.U)} paddr=0x${Hexadecimal(io.s1_paddr)}\n")
    }
  }
  ...
  val s2_crypto_idx = RegEnable(index(s1_vaddr, io.s1_paddr), s1_clk_en)
  val s2_crypto_block = RegEnable(io.s1_paddr >> log2Ceil(cacheBlockBytes), s1_clk_en)
  ...
  outer.icacheParams.latency match {
    ...
    case 1 =>
      ...
      val s1RespRawData = Mux1H(s1_tag_hit, s1_dout)
      io.resp.bits.data := s1RespRawData
      val s1CryptoIdx = index(s1_vaddr, io.s1_paddr)
      val s1CryptoNonce = Mux1H(s1_tag_hit, VecInit((0 until nWays).map(i => l1NonceMeta(s1CryptoIdx)(i))))
      val s1CryptoBlock = io.s1_paddr >> log2Ceil(cacheBlockBytes)
      // Legacy local-hit decrypt path (disabled): decrypt now happens in L2 SourceD before refill reaches L1.
      // val s1CryptoPlain = CacheCrypto.cryptBeat(s1RespRawData, CacheCrypto.nonceEff(s1CryptoNonce, s1CryptoBlock), 0.U)
      // io.resp.bits.data := Mux(io.cacheCryptoEnable, s1CryptoPlain, s1RespRawData)
      io.resp.bits.data := s1RespRawData
      io.resp.bits.ae := s1_tl_error.asUInt.orR
      io.resp.valid := s1_valid && s1_hit
      io.resp.bits.replay := false.B
      when (io.cacheCryptoEnable && io.resp.valid) {
        printf(p"[ICacheCrypto] resp-no-local-decrypt-l1 set=${Hexadecimal(s1CryptoIdx)} nonce=0x${Hexadecimal(s1CryptoNonce)} block=0x${Hexadecimal(s1CryptoBlock)}\n")
      }
    ...
    case 2 =>
      ...
      val s2RespRawData = s2_data_decoded.uncorrected
      io.resp.bits.data := s2RespRawData
      val s2CryptoNonce = l1NonceMeta(s2_crypto_idx)(s2_hit_way)
      // Legacy local-hit decrypt path (disabled): decrypt now happens in L2 SourceD before refill reaches L1.
      // val s2CryptoPlain = CacheCrypto.cryptBeat(s2RespRawData, CacheCrypto.nonceEff(s2CryptoNonce, s2_crypto_block), 0.U)
      // io.resp.bits.data := Mux(io.cacheCryptoEnable, s2CryptoPlain, s2RespRawData)
      io.resp.bits.data := s2RespRawData
      io.resp.bits.ae := s2_tl_error
      io.resp.bits.replay := s2_disparity
      io.resp.valid := s2_valid && s2_hit
      when (io.cacheCryptoEnable && io.resp.valid) {
        printf(p"[ICacheCrypto] resp-no-local-decrypt-l2 set=${Hexadecimal(s2_crypto_idx)} way=${Decimal(s2_hit_way)} nonce=0x${Hexadecimal(s2CryptoNonce)} block=0x${Hexadecimal(s2_crypto_block)}\n")
      }
