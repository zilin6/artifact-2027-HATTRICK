...
trait HasMissInfo extends Bundle with HasL1HellaCacheParameters {
  ...
  val old_counter = UInt(p(CacheCryptoCounterBitsKey).W)
...
class MSHR(id: Int)(implicit edge: TLEdgeOut, p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  io.meta_write.bits.data := 0.U.asTypeOf(new L1Metadata)
...
class ProbeUnit(implicit edge: TLEdgeOut, p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  io.wb_req.bits.cryptoLine := false.B
...
class NonBlockingDCacheModule(outer: NonBlockingDCache) extends HellaCacheModule(outer) {
  ...
  when (dtlb.io.req.valid && watchPtwPhysReq) {
    chisel3.printf("[NBD-PTW-S1] addr=0x%x page=0x%x passthrough=%d cmd=0x%x size=%d dprv=%d dv=%d req_ready=%d resp_miss=%d resp_ae_ld=%d resp_pf_ld=%d resp_ma_ld=%d resp_cacheable=%d\n",
      s1_req.addr,
      s1_req.addr(paddrBits-1, pgIdxBits),
      dtlb.io.req.bits.passthrough,
      dtlb.io.req.bits.cmd,
      dtlb.io.req.bits.size,
      dtlb.io.req.bits.prv,
      dtlb.io.req.bits.v,
      dtlb.io.req.ready,
      dtlb.io.resp.miss,
      dtlb.io.resp.ae.ld,
  ...
  mshrs.io.req.bits.old_meta := Mux(s2_tag_match, L1Metadata(s2_repl_meta.tag, s2_hit_state, s2_repl_meta.reenc_active, s2_repl_meta.cryptoLine), s2_repl_meta)
  mshrs.io.req.bits.old_counter := 0.U
  ...
  val watchPtwPhysResp =
    s2_req.phys && s2_req.cmd === M_XRD
  when (RegNext(s1_xcpt_valid) && watchPtwPhysResp) {
    chisel3.printf("[NBD-PTW-S2] addr=0x%x page=0x%x xcpt_ae_ld=%d xcpt_pf_ld=%d xcpt_ma_ld=%d xcpt_ae_st=%d resp_data_valid=%d ordered=%d\n",
      s2_req.addr,
      s2_req.addr(paddrBits-1, pgIdxBits),
      io.cpu.s2_xcpt.ae.ld,
      io.cpu.s2_xcpt.pf.ld,
      io.cpu.s2_xcpt.ma.ld,
      io.cpu.s2_xcpt.ae.st,
      io.cpu.resp.valid,
      io.cpu.ordered)
  }
