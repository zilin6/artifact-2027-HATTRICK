...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  val cacheCryptoNonceBits = p(CacheCryptoNonceBitsKey)
  val cacheCryptoCounterBits = p(CacheCryptoCounterBitsKey)
  val cacheCryptoCounterMax = ((BigInt(1) << cacheCryptoCounterBits) - 1).U(cacheCryptoCounterBits.W)
  val l1NonceMeta = RegInit(VecInit(Seq.fill(nSets)(VecInit(Seq.fill(nWays)(0.U(cacheCryptoNonceBits.W))))))
  val l1CtrMeta = RegInit(VecInit(Seq.fill(nSets)(VecInit(Seq.fill(nWays)(0.U(cacheCryptoCounterBits.W))))))
  val l1BusyMeta = RegInit(VecInit(Seq.fill(nSets)(VecInit(Seq.fill(nWays)(false.B)))))
  ...
  val s1CryptoSet = s1_vaddr(idxMSB, idxLSB)
  val s1CryptoBusy = (s1_hit_way.asBools zip l1BusyMeta(s1CryptoSet)).map { case (h, b) => h && b }.foldLeft(false.B)(_||_)
  when (io.cacheCryptoEnable && s1_valid_masked && s1_hit_way.orR && s1CryptoBusy) {
    s1_nack := true.B
    printf(p"[DCacheCrypto] busy-hit-nack set=${Hexadecimal(s1CryptoSet)} wayOH=${Hexadecimal(s1_hit_way)} addr=0x${Hexadecimal(s1_paddr)}\n")
  }
  ...
  val s2CryptoBusy = l1BusyMeta(s2_vaddr(idxMSB, idxLSB))(OHToUInt(s2_hit_way))
  ...
  val s2_dont_nack_misc = s2_valid_masked && !s2_meta_error &&
    (supports_flush.B && s2_cmd_flush_all && flushed && !flushing ||
     ...
     s2_req.cmd === M_WOK)
  val s2NackRaw = s2_valid_no_xcpt && !s2_dont_nack_uncached && !s2_dont_nack_misc && !s2_valid_hit
  io.cpu.s2_nack := s2NackRaw
  io.cpu.s2_nack := Mux(io.cacheCryptoEnable, s2NackRaw || (s2_valid_masked && s2_hit && s2CryptoBusy), s2NackRaw)
  ...
  val pstoreRawValid = should_pstore_drain(false.B)
  dataArb.io.in(0).valid := pstoreRawValid
  ...
  val pstoreRawWdata = encodeData(Fill(rowWords, Mux(pstore2_valid, pstore2_storegen_data, pstore1_data)), false.B)
  dataArb.io.in(0).bits.wdata := pstoreRawWdata
  val pstoreMetaSet = Mux(pstore2_valid, pstore2_addr(idxMSB, idxLSB), pstore1_addr(idxMSB, idxLSB))
  val pstoreMetaWayOH = Mux(pstore2_valid, pstore2_way, pstore1_way)
  val pstoreMetaWay = OHToUInt(pstoreMetaWayOH)
  val pstoreMetaBusy = l1BusyMeta(pstoreMetaSet)(pstoreMetaWay)
  val pstorePlainWord = Mux(pstore2_valid, pstore2_storegen_data, pstore1_data)
  val pstoreNonce = l1NonceMeta(pstoreMetaSet)(pstoreMetaWay)
  val pstoreBlock = (Mux(pstore2_valid, pstore2_addr, pstore1_addr) >> blockOffBits)
  val pstoreCipherWord = CacheCrypto.cryptBeat(pstorePlainWord, CacheCrypto.nonceEff(pstoreNonce, pstoreBlock), 0.U)
  val pstoreCipherEnc = encodeData(Fill(rowWords, pstoreCipherWord), false.B)
  dataArb.io.in(0).bits.wdata := Mux(io.cacheCryptoEnable, pstoreCipherEnc, pstoreRawWdata)
  dataArb.io.in(0).valid := Mux(io.cacheCryptoEnable, pstoreRawValid && !pstoreMetaBusy, pstoreRawValid)
  when (io.cacheCryptoEnable && pstoreRawValid && pstoreMetaBusy) {
    printf(p"[DCacheCrypto] pstore-stall-busy set=${Hexadecimal(pstoreMetaSet)} way=${Decimal(pstoreMetaWay)} addr=0x${Hexadecimal(Mux(pstore2_valid, pstore2_addr, pstore1_addr))}\n")
  }
  ...
  val pstoreNonceNew = random.LFSR(cacheCryptoNonceBits max 16, dataArb.io.in(0).fire)
  when (dataArb.io.in(0).fire && dataArb.io.in(0).bits.write && io.cacheCryptoEnable) {
    val ctrNow = l1CtrMeta(pstoreMetaSet)(pstoreMetaWay)
    when (ctrNow === cacheCryptoCounterMax) {
      l1BusyMeta(pstoreMetaSet)(pstoreMetaWay) := true.B
      l1NonceMeta(pstoreMetaSet)(pstoreMetaWay) := pstoreNonceNew(cacheCryptoNonceBits-1, 0)
      l1CtrMeta(pstoreMetaSet)(pstoreMetaWay) := 1.U
      l1BusyMeta(pstoreMetaSet)(pstoreMetaWay) := false.B
      printf(p"[DCacheCrypto] ctr-rollover set=${Hexadecimal(pstoreMetaSet)} way=${Decimal(pstoreMetaWay)} oldCtr=${Decimal(ctrNow)} newNonce=0x${Hexadecimal(pstoreNonceNew(cacheCryptoNonceBits-1, 0))}\n")
    }.otherwise {
      l1CtrMeta(pstoreMetaSet)(pstoreMetaWay) := ctrNow + 1.U
      l1BusyMeta(pstoreMetaSet)(pstoreMetaWay) := false.B
      printf(p"[DCacheCrypto] ctr-inc set=${Hexadecimal(pstoreMetaSet)} way=${Decimal(pstoreMetaWay)} oldCtr=${Decimal(ctrNow)} newCtr=${Decimal(ctrNow + 1.U)}\n")
    }
  }
  ...
  tl_out_a.bits.user.lift(CacheCryptoWritebackMeta).foreach { u =>
    u.counter := 0.U
  ...
  val refillMetaNonce = WireDefault(0.U(cacheCryptoNonceBits.W))
  val refillMetaCounter = WireDefault(0.U(cacheCryptoCounterBits.W))
  val refillMetaBusy = WireDefault(false.B)
  refillCryptoMeta.foreach { m =>
  ...
  when (grantIsCached && d_done && !tl_out.d.bits.denied) {
    val refillSet = s2_vaddr(idxMSB, idxLSB)
    val refillWay = OHToUInt(refill_way)
    l1NonceMeta(refillSet)(refillWay) := Mux(io.cacheCryptoEnable, refillMetaNonce, 0.U)
    l1CtrMeta(refillSet)(refillWay) := Mux(io.cacheCryptoEnable, refillMetaCounter, 0.U)
    l1BusyMeta(refillSet)(refillWay) := Mux(io.cacheCryptoEnable, refillMetaBusy, false.B)
    when (io.cacheCryptoEnable) {
      printf(p"[DCacheCrypto] refill-meta set=${Hexadecimal(refillSet)} way=${Decimal(refillWay)} nonce=0x${Hexadecimal(refillMetaNonce)} ctr=${Decimal(refillMetaCounter)} busy=${refillMetaBusy}\n")
    }
  }
  ...
  tl_out_c.bits.user.lift(CacheCryptoWritebackMeta).foreach { u =>
    u.counter := 0.U
    u.cryptoLine := false.B
  }
  ...
  if (!usingDataScratchpad) {
    ...
    val releaseSetIdx = probeIdx(probe_bits)
    val releaseWayIdx = OHToUInt(releaseWay)
    val releaseNonceMeta = l1NonceMeta(releaseSetIdx)(releaseWayIdx)
    val releaseCtrMeta = l1CtrMeta(releaseSetIdx)(releaseWayIdx)
    val releaseBusyMeta = l1BusyMeta(releaseSetIdx)(releaseWayIdx)
    tl_out_c.bits.user.lift(CacheCryptoWritebackMeta).foreach { u =>
      u.counter := Mux(io.cacheCryptoEnable, releaseCtrMeta, 0.U)
      u.cryptoLine := io.cacheCryptoEnable
    }
    when (io.cacheCryptoEnable && tl_out_c.fire && c_first) {
      printf(p"[DCacheCrypto] wb-meta set=${Hexadecimal(releaseSetIdx)} way=${Decimal(releaseWayIdx)} nonce=0x${Hexadecimal(releaseNonceMeta)} ctr=${Decimal(releaseCtrMeta)} busy=${releaseBusyMeta}\n")
    }
  }
  ...
  val respValidRaw = (s2_valid_hit_pre_data_ecc || doUncachedResp) && !s2_data_error
  io.cpu.resp.valid := respValidRaw
  io.cpu.resp.valid := Mux(io.cacheCryptoEnable, respValidRaw && !s2CryptoBusy, respValidRaw)
  when (io.cacheCryptoEnable && respValidRaw && s2CryptoBusy) {
    printf(p"[DCacheCrypto] resp-blocked-busy set=${Hexadecimal(s2_vaddr(idxMSB, idxLSB))} way=${Decimal(OHToUInt(s2_hit_way))} addr=0x${Hexadecimal(s2_req.addr)}\n")
  }
  ...
  val s2CryptoSet = s2_vaddr(idxMSB, idxLSB)
  val s2CryptoWay = OHToUInt(s2_hit_way)
  val s2CryptoNonce = l1NonceMeta(s2CryptoSet)(s2CryptoWay)
  val s2CryptoBlock = s2_req.addr >> log2Ceil(cacheBlockBytes)
  val s2CryptoWord = CacheCrypto.cryptBeat(s2_data_word_possibly_uncached, CacheCrypto.nonceEff(s2CryptoNonce, s2CryptoBlock), s2_word_idx)
  val loadgenCrypto = new LoadGen(s2_req.size, s2_req.signed, s2_req.addr, s2CryptoWord, s2_sc, wordBytes)
  io.cpu.resp.bits.data := Mux(io.cacheCryptoEnable && s2_valid_hit_pre_data_ecc, loadgenCrypto.data | s2_sc_fail, io.cpu.resp.bits.data)
  when (io.cacheCryptoEnable && s2_valid_hit_pre_data_ecc && s2_read) {
    printf(p"[DCacheCrypto] load-decrypt set=${Hexadecimal(s2CryptoSet)} way=${Decimal(s2CryptoWay)} block=0x${Hexadecimal(s2CryptoBlock)} nonce=0x${Hexadecimal(s2CryptoNonce)} wordIdx=${Decimal(s2_word_idx)}\n")
  }
  io.cpu.resp.bits.data_word_bypass := loadgen.wordData
  val s2DataRaw = s2_data_word
  io.cpu.resp.bits.data_raw := s2DataRaw
  io.cpu.resp.bits.data_raw := Mux(io.cacheCryptoEnable && s2_valid_hit_pre_data_ecc, s2CryptoWord, s2DataRaw)
  ...
  when (resetting) {
    ...
    for (set <- 0 until nSets) {
      for (way <- 0 until nWays) {
        l1NonceMeta(set)(way) := 0.U
        l1CtrMeta(set)(way) := 0.U
        l1BusyMeta(set)(way) := false.B
      }
    }
    when (flushDone) {
      resetting := false.B
